...
GitHub·이전 호스팅·Neon·네이버 클라우드·Expo/EAS·카카오·App Store Connect·Google Play의 실제 계정/프로젝트 접근을 확인했다. 브라우저 로그인과 서비스 리소스 조회까지 확보한 상태이며, 모든 외부 서비스의 무인 API 인증을 새로 설치한 상태는 아니다.

**이 문단은 2026-09-11 대표 지시로 뒤집혔다 — `weddingpick.kr`은 폐기했다. 아래는 2026-09-10 시점의 기록이다.** 당시 확정: `weddingpick.kr`은 보유한 커스텀 도메인이지만 현재 사용하지 않는다. DNS 미응답을 장애·연결 차단으로 분류하지 않는다. 도메인 구매·복구·Cloudflare 등록은 진행하지 않았다.
...
| 이전 호스팅 | 웨딩픽 프로젝트, Production 리소스 5개, API·앱 웹·웹사이트 배포 상세와 설정 조회 | 로그인된 콘솔. 직접 이전 호스팅 API 키를 로컬에 설치하지 않음 |
...
| 대상 | 확인한 현재 버전 |
|---|---|
| 이전 호스팅 운영 API | `e34193e`, Live, 9월 10일 22:19:16 KST, Auto-Deploy |
| 이전 호스팅 앱 웹·관리자 | `e34193e`, Live, 9월 10일 22:19:16 KST |
| 이전 호스팅 웹사이트 | `e34193e`, Live, 9월 10일 22:19:16 KST |
...
공개 `https://210.109.82.212/health`를 실제 호출한 결과:
...
Neon 콘솔에서는 production 브랜치 하나에 `neondb`와 `weddingpick_staging` 두 DB를 확인했다. 별도 DB 이름은 있지만 별도 브랜치로 환경이 분리된 구성은 아니다. 이전 호스팅에서는 Production에 API·앱 웹·웹사이트·별도 DB·별도 관리자 리소스가 있고, Webview 환경은 비어 있다. 남아 있는 별도 DB/관리자 리소스를 이름만 보고 사용 중·미사용으로 단정하거나 삭제하지 않았다.

## 속도와 환경 차이

- 이전 호스팅 운영 API는 **Free**, 리전 **Ohio (US East)**다. 콘솔에는 유휴 후 기동으로 요청이 **50초 이상 지연될 수 있음**이 명시돼 있다. 이는 서비스 설정에 표시된 가능성이며 이번에 재현·측정한 지연 시간이 아니다.
...
- `삭제된 이전 호스팅 환경 선언:62,67`: 커스텀 도메인 설명과 CORS allowlist에 잔존.
...
실제 활성 주소는 API `210.109.82.212`, 앱/관리자 `210.109.82.212`, 웹사이트 `210.109.82.212`이다.
