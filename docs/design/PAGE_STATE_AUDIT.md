...
- `empty` 하나로 기본 상태 검수를 끝내지 않는다. 최초 기본·필수 정보 미설정·조회 성공 0건·필터 결과 없음·일부 정보 부족·정상 0을 별도 기록한다. 공통 표현 기준은 핸드오프 §8-1, 검수 fixture는 이 문서 §8이다.
- 앱 캡처는 390×844, 관리자는 정본의 1920×1080을 우선 기준으로 한다. WEB은 해당 시안의 화면 크기로 별도 대조한다.
...
| WP-ADM-001 | 관리자 홈 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-002 | 일일 브리핑 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-010 | 제보 처리 현황 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-011 | 확인 필요 목록 | ADM | 미매핑 | 실제 라우트/컴포넌트/통합·삭제 여부 판정 후 연결 |
| WP-ADM-012 | 가격 통계 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-013 | 이상치 · 조작 탐지 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-014 | 업체 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-015 | 이미지 자동 수급 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-016 | 이메일 회신 자동 매칭 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-020 | 사용자 계정 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-021 | 고객 의견 · 문의 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-022 | 후기 · 반론 관리 | ADM | 미매핑 | 실제 라우트/컴포넌트/통합·삭제 여부 판정 후 연결 |
| WP-ADM-023 | 업체 문의 처리 목록 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-030 | 마케팅 자동화 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-031 | 캠페인 · 보상 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-032 | 수익 현황 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-033 | 광고 집행 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-034 | 광고 실운영 전환 조건 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-035 | 자주 묻는 질문 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-036 | 약관 · 방침 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-040 | 자동화 상태 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-041 | 긴급 중지 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-042 | 변경 복구 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-050 | 인공지능 사용량 · 비용 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-051 | 정책 규칙 관리 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
| WP-ADM-052 | 감사 기록 | ADM | 라우트 매핑 | 기준 시안 위치·실제 상태·1920×1080 대조 증거 연결 |
...
검색·필터 no-results:
일부 정보 부족 / 정상 0:
페이지·섹션·필드 상태 표현과 CTA 중복 여부:
error / retry:
refreshing:
submitting / double-submit:
세션 / 권한 / 직접 링크:
Back / 탭 / 필터 / 스크롤 복원:
정본 캡처와 화면 크기 (앱 390×844 / 관리자 1920×1080):
동일 크기·동일 상태 실제 캡처:
시각 차이 / 조치:
넓은 웹 / 네이티브 / 접근성:
미확인 / N/A 사유:
...
## 8. 기본·정보 없음 상태 공통 검수 행렬

사용자 추가 요청: **정보가 없거나 완전 기본 상태인 화면도 전체 페이지에서 일관되게 표현한다.** 아래 fixture는 활성 화면마다 적용 여부를 판정하고, 대상이 아니면 근거를 남긴다. 신규 화면 ID/route 추가가 아니라 기존 ID의 상태 검수다.

| Fixture | 입력 조건 | 반드시 확인할 결과 |
| --- | --- | --- |
| DEFAULT | 인증·필수 온보딩 완료, 개인 사용 이력 없음, 선택 설정 미지정 | 화면 목적·첫 행동·헤더 유지. 공용 업체/글과 자동 생성 준비 항목은 정상 표시 |
| UNSET | 예식일/예산/배우자 연결 등 일부 설정만 비어 있음 | 항목별 독립 안내. 임의 날짜·0원·가짜 배우자 표시 금지 |
| EMPTY | 조회 성공, 실제 목록 0건 | 로더 종료, 같은 템플릿의 공통 안내·정렬·CTA 규칙 |
| NO_RESULTS | 검색어/필터 조건의 조회 성공 0건 | 검색어·조건 유지, 전체 자료 없음으로 단정하지 않음 |
| PARTIAL | 업체 정보 있음, 사진/제보 금액/후기 중 일부 없음 | 해당 필드/섹션만 정보 없음. 사용 가능한 본문 유지 |
| VALID_ZERO | 지출 0원·목록 건수 0 등 성공 응답의 유효한 0값 | unknown/null로 치환하지 않음. 미확인 알림 0건으로 전체 알림을 숨기지 않음 |
| ERROR_INITIAL | 첫 조회 실패 | empty 대신 오류·유효한 재시도. 로더 무한 지속 없음 |
| ERROR_REFRESH | 정상 내용 표시 후 재조회 실패 | 기존 내용·입력·선택 유지, 작은 실패 안내 |
| COMPLETE | 실제 최종 결정/완료 근거가 있음 | empty와 완료 구분. 최종 Pick 뒤에만 상담 예약 연결 |
| UNSUPPORTED | 권한 없음 또는 기능 미지원 | 원인과 가능한 행동만, 무의미한 로더·재시도·CTA 없음 |

### 기본 계정으로 함께 볼 화면

- 홈: 예식 정보·준비 항목·추천·예산·웨딩 준비 팁을 같은 계정으로 비교. 예산/팁 섹션 유지.
- Pick/비교: 후보 없음·비교함 없음·비교함 한 곳·최종 결정 없음 구분. 예약 CTA 선행 노출 금지.
- 웨딩노트: 사용자 등록 일정·지출·메모가 없을 때 같은 상태 표현. 서버가 만든 준비 항목은 임의로 없애지 않음.
- 검색/업체/추천: 공용 자료는 개인 이력과 독립. 필터 0건·금액 수집 중·사진 미등록·준비 완료를 분리.
- MY/알림/스크랩/혜택: 미입력·내역 없음·미확인 0건·조건 미충족·실패를 구분. 도움 안 되는 CTA 생성 금지.
- 등록/추가/작성: empty 목록 화면 대신 입력 가능한 기본 폼. 기존 BottomSheet·dirty-close·Back 규칙 유지.
- 관리자: 정상 대기열 0건·수집 장애·읽기 전용·미지원 분리. 표 머리와 shell을 유지.
- WEB/시트/공통 상태: 적용 가능한 상태만 검증. N/A 항목은 시안·기능 근거를 남김.

### 비교 기록

같은 템플릿에 속한 화면은 상태 블록 시작 위치, 정렬, 타이포, 배경, 아이콘 사용, 버튼 변형을 나란히 비교한다. 서로 다른 템플릿을 억지로 같은 전체 화면으로 바꾸지 않는다.

각 화면 기록에는 다음을 추가한다.

```text
상태 판정 근거 / API 필드:
기본 필터·폼 값 / 사용자 입력과 구분:
적용 fixture / 미적용 사유:
표시 범위: 페이지 / 섹션 / 필드 / 폼 / 표
제목 / 안내 / CTA / 목적지:
사용한 공통 컴포넌트·토큰:
