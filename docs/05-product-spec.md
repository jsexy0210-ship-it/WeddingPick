...
| # | 화면 | 내용 | Phase |
|---|---|---|---|
| A-01 | 온보딩 | 서비스 소개 3~4스텝, 카메라/알림 권한 안내 | 1 |
...
PriceStat
  ...
  sampleCount, median, p25, p75
  minVerificationLevel                 // L2 이상만 집계
  recomputedAt
```
- 사업계획서 27번 원칙에 따라 **중앙값·분포·판단은 전부 서버 계산**. AI 출력은 여기 직접 들어가지 않는다.
- 판단 표현 4단계(낮은 편 / 비슷한 수준 / 다소 높은 편 / 높은 편) 임계값 `결정 필요` — 제안: 사분위 기준(p25 미만 / p25~p75 / p75~p90 / p90 초과)
- `sampleCount`와 `periodStart~periodEnd`는 노출 시 **항상 동반 표시** (사업계획서 9번)
...
1인 운영·90~95% 자동처리(사업계획서 35번) 목표를 전제로, 운영 부담이 가장 적은 조합을 제안한다.

| 영역 | 제안 | 근거 |
|---|---|---|
| 모바일 앱 | React Native + Expo (TypeScript) | 카메라·파일 선택·푸시가 표준 모듈로 해결됨. iOS/Android 동시 대응. OTA 업데이트로 1인 운영 부담 감소 |
| 웹 | Next.js 정적 랜딩 | 웹 역할은 소개·다운로드·정책 게시로 한정 (서비스정책서 9번). 앱 기능 재현 금지 |
| 백엔드 | TypeScript API 서버 (앱과 언어 통일) | 스키마·타입 공유. `결정 필요`: 런타임/호스팅 |
...
`결정 필요` 항목:
- [ ] 백엔드 호스팅 (관리형 PaaS / 서버리스 / 컨테이너)
- [ ] 인증 제공자 (Apple 필수, Kakao/Google 여부)
- [ ] 리전 — 계약서 원본을 다루므로 국내 리전 우선 검토 권장
- [ ] 푸시 발송 채널
- [ ] Flutter를 선택할 경우 위 모바일/웹/백엔드 언어 통일 이점은 사라짐 (선택 가능하나 근거 재확인 필요)
...
| 순번 | 내용 | 완료 기준 |
|---|---|---|
| 1 | 레포 구조 + 앱 스캐폴드 + CI | 빈 앱이 iOS/Android에서 실행 |
| 2 | 인증 + Wedding 생성 | 로그인 후 내 웨딩 진입 |
| 3 | 촬영·업로드 + 원본 저장소 분리 | 원본이 분리 버킷에 저장, retentionUntil 기록 |
| 4 | AI 분석 (분류·추출·신뢰도) | 실제 견적서 샘플에서 핵심 필드 추출 |
| 5 | 확인 단계 + 결과 화면 | 핵심 필드 확인 없이 저장 불가 |
| 6 | 개인정보 탐지·마스킹 + 자동삭제 배치 | 보관기간 경과 원본이 실제로 삭제, 실패 시 알림 |
| 7 | 검증 등급 + 인증 신청 | L0~L2 부여 및 배지 표시 |
| 8 | PriceStat 집계 + 가격 비교 화면 | 표본 부족 시 중앙값 미노출 동작 확인 |
| 9 | 정책 화면 + 웹 랜딩 | 약관·개인정보처리방침·AI 안내 게시 |
...
- [ ] 중앙값 노출 최소 표본 수
- [ ] 가격 판단 4단계 임계값
- [ ] 상품 동일성 판단 기준 — "같은 상품"을 무엇으로 볼 것인가 (업체+상품명? 옵션 조합?). 중앙값 신뢰도를 좌우하는 핵심 정의
