...
| A-16 ✅ | 검색 | 업체 탐색 — 이름·분류·지역<br>플래너 탐색은 미착수. 개인 이름을 목록으로 공개하는 문제라 법률 검토가 먼저다 | 2 |
...
남은 것은 대부분 화면이 아니라 바깥에서 와야 하는 값이다: A-02는 Apple·Kakao 클라이언트 ID가 나와야 실제 제공자로 로그인한다(그 전까지는 서버가 켜둔 개발용 경로만 보이고, 화면이 "실제 애플·카카오 로그인이 아니다"라고 밝힌다). A-13은 접수까지 되고 승인은 사람이 증빙을 확인해야 난다. A-16의 플래너 탐색은 아직 만들지 않았다.
...
**플래너 탐색은 아직 없다.** 사업계획서 11번은 플래너를 독립 비교대상으로 보지만, 개인의
이름을 검색 목록으로 공개하는 것은 업체를 공개하는 것과 다른 문제다. 법률검토 결과가
나온 뒤에 만든다. `결정 필요`.
...
- [ ] 플래너 검색 공개 여부 — 개인 이름을 목록으로 공개하는 문제다(11번). 법률검토 결과가 필요하다
