...
| 4 🚧 | AI 분석 (분류·추출·신뢰도) | 실제 견적서 샘플에서 핵심 필드 추출<br>워커 완료 — 분류·추출·신뢰도·개인정보 종류까지. 실제 견적서로 정확도를 확인하지는 못했다 |
| 5 🚧 | 확인 단계 + 결과 화면 | 핵심 필드 확인 없이 저장 불가<br>서버 쪽 완료(API·DB 트리거 이중). 화면 A-07·A-08이 남았다 |
...
| 8 🚧 | PriceStat 집계 + 가격 비교 화면 | 표본 부족 시 중앙값 미노출 동작 확인<br>비교 API는 완료(표본 부족 시 이유 반환). 화면은 분석 이후 |
