...
**업체 자료 확보**: 지방행정 인허가 데이터(공개 자료)로 업체를 등록한다. 특정 사이트를 긁어오지 않는다(사업계획서 20번). 영업 중인 곳만 넣고, 지역은 시도·시군구까지만, 자료의 갱신일자를 마지막 확인일로 남긴다. 등록하면 그 이름으로 기다리던 견적이 자동 연결된다. `결정 필요`: 데이터셋 이용허락범위에 따른 출처 표시 문구.
