...
| A-17 🚧 | 업체 상세 / 업체 비교 | 업체 상세 완료 — 상품별 실제 계약 중앙값·표본 수·기준 기간<br>최대 3곳 비교는 미착수 | 2 |
...
남은 것은 대부분 화면이 아니라 바깥에서 와야 하는 값이다: A-02는 Apple·Kakao 클라이언트 ID가 나와야 실제 제공자로 로그인한다(그 전까지는 서버가 켜둔 개발용 경로만 보이고, 화면이 "실제 애플·카카오 로그인이 아니다"라고 밝힌다). A-13은 접수까지 되고 승인은 사람이 증빙을 확인해야 난다. A-16의 플래너 탐색과 A-17의 3곳 비교는 아직 만들지 않았다.
