...
### ⓒ `fab` 아이콘 — **부품으로는 닫는다. 아이콘만 「피그마 그림 미확인」**

56 · 곡률 · 그림자가 피그마와 이미 같다(`our-wedding.txt` «button 56×56 · r9999 · shadow»).
**다른 것은 속뿐이다** — 규격서는 «svg 24×24»라고 적지만 **무엇을 그린 svg인지는
규격서가 말하지 않는다.** SEED도 `icon.size` 24만 말한다. 우리는 글자 글리프를 쓴다
(`glyph="✎"` · 기본 `"+"`).

`ProductSymbol`에도 `SeedIcon`에도 더하기 · 연필 아이콘이 없다. **아이콘은 「무엇을 하는
단추인가」를 말하는 자리라 규격만 보고 갈아끼우면 뜻이 달라진다** — 바꾸지 않았다
(2026-09-15 MASTER 확정).

`scripts/screenshot-figma.mjs`로 그 화면을 찍어 무엇을 그렸는지 보면 그때 고친다.
...
**전부 닫혔다.** 남는 것은 대표 판단 둘뿐이다 — `xlarge` 52를 56으로 올릴지(ⓗ), `fab` 아이콘(ⓒ 피그마 그림 미확인).
