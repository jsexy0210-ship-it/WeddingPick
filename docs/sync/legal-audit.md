...
> **[2026-09-16 전제 변경]** 대표 지시 — 「개인정보처리방침 이용약관 마케팅 약관도 동일하게
> 내가 수정가능하도록 하고」. **정본이 «하나»인 것은 그대로지만 그 하나가 코드에서 표로
> 옮겨간다.** 아래 「정본은 `apps/web/src/subpages.ts` 하나다」는 그때까지의 상태를 적은 것이고,
> 옮기는 작업이 `claude/editable-legal-docs`에서 돌고 있다. **감사 방법은 그대로 유효하다** —
> 「적힌 문장과 코드가 실제로 하는 일을 맞대어 본다」. 볼 곳이 파일에서 표로 바뀔 뿐이다.
