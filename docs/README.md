...
```
1  CLAUDE.md                                   저장소 뿌리. 고정 규칙
2  design-handoff/current/CHANGELOG.md 맨 위    현 기준 v3.27 · 최신이 위
3  design-handoff/current/  SPEC · PROJECT_RULES · screens.json · tokens.json
4  통합정책 v3.15                                위 셋이 다루지 않는 사업·정책 범위
```

화면·용어·값이 다투면 **1~3이 이긴다.** 「무조건 최신 핸드오프 md 기준으로 바꾼다」가
사용자 지침이다(2026-09-08). 통합정책은 위가 다루지 않는 자리에서만 기준이 된다.

**버전을 기억으로 말하지 않는다.** 화면에 손대기 전에 `CHANGELOG.md` 맨 위를 그때그때 읽는다
(2026-09-10 사용자 지침). 이 README에 적힌 v3.27과 그 파일이 어긋나면 파일이 맞다.

2026-09-10 이전에는 이 자리가 「통합정책 v3.15가 현재 확정 기준」이라고만 적혀 있었다.
그 사이 핸드오프가 v3.27까지 열두 판을 지나갔고, 이 문서를 먼저 연 세션은 폐기된 값을
현행으로 읽을 수 있었다.
