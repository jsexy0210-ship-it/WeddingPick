...
**추가 지시(2026-09-09): 「우선 현재 DB 그대로 사용한다. 차후에 분리.」**
그래서 아래 「옮기는 순서」의 0 · 0b · 1은 **지금 하지 않는다.** 나누기로 정한 날 시작한다.
지금 유효한 것은 2(잡 이름 정정)와 3(시드 차단 · PR #134로 완료)뿐이다.

이 결정이 바꾸는 것이 하나 더 있다. **수집 크론을 스테이징으로 돌리는 것은 지금 의미가 없다** —
DB가 한 벌이므로 스테이징으로 보낸 수집 결과는 앱에 닿지 않고, 그 스테이징 DB는 19개 밀려 있어
반영 자체가 실패한다. 무인 실행을 막는 목적은 그대로 두되, 방법은 **크론을 끄고 수동 실행만
남기는 것**으로 한다. 수집기가 검증되면(업종 코드 확정 · 0건 가드 · 건수 상한) 사람이 골라 돌린다.

## 1. 지금 상태 — 이름과 실제가 어긋나 있다

| 이름 | 실제로 가리키는 것 | 근거 |
|---|---|---|
| `DATABASE_URL` | **운영 DB** | `삭제된 이전 호스팅 환경 선언`이 운영 API 서비스(`WeddingPickl`)에 이 값을 넣는다. `db-migrate-staging.yml` 머리 주석도 「`db-migrate.yml`의 `DATABASE_URL`(운영 DB)」이라고 적었다 |
| `STAGING_DATABASE_URL` | 스테이징 DB | `db-migrate-staging.yml`만 쓴다. 자동 파이프라인은 건드리지 않는다 |
| `PRODUCTION_DATABASE_URL` | **밖에서는 못 쓴다** | 2026-09-09 `db-status.yml` 실행 결과, GitHub Actions에서 주소를 찾지 못하고 끝났다(`getaddrinfo EAI_AGAIN`). 이전 호스팅 내부망에서만 풀리는 이름이 들어 있다 |
...
| `DATABASE_URL` | 접속됨. **적용 95 / 기대 92** — 밀린 것 없음. 대신 저장소에 파일이 없는 기록이 3개 있다 |
...
이전 호스팅 쪽도 한 벌뿐이다. `삭제된 이전 호스팅 선언`의 네 서비스(`weddingpick-web` · `weddingpick-admin` ·
...
| `삭제된_배포_API_키` | 환경변수 동기화 · 배포 상태 · 수동 배포 | 양쪽 공용 |
...
이전 호스팅 서비스 안에서만 쓰는 내부 주소가 따로 필요하면 `render-env.yml` 쪽에 다른 이름으로 둔다.

### 환경변수 선언

`삭제된 이전 호스팅 환경 선언`을 환경별로 가른다. 서비스 이름이 다르므로 블록을 하나 더 두면 된다.
...
나누는 날 한 번에 하지 않는다. **위 추가 지시로 0 · 0b · 1은 나누는 날로 미룬다.**
...
4  서비스 생성    이전 호스팅에 스테이징 4종 · 스테이징 Neon DB
...
지금 하는 것은 2뿐이고 3은 PR #134로 끝났다. 0 · 0b · 1은 4~8과 같은 날 한다 —
0을 먼저 하지 않고 1을 하면 **모든 워크플로가 운영 DB에 닿지 못한다**(1절 측정).

## 4. 아직 못 정한 것

- 스테이징 이전 호스팅 서비스를 무료 플랜으로 둘지. 무료면 15분마다 잠들어 검수 때마다 기다린다.
