...
**약관·방침의 직접 조작을 어디에 두는가.** 정본은 웹사이트다(2026-09-11 대표 지시).

- **안 1 · 관리자에서 정본을 고친다.** `admin/terms`를 실제 편집 화면으로 만들고, 웹 빌드가
  API에서 조문을 읽어 그린다. 운영자가 배포 없이 고칠 수 있다. 대신 웹의 약관이 빌드 시점
  API 응답에 묶이고, 지금 `subpages.ts`에 든 본문을 DB로 옮기는 이전 작업이 따른다.
- **안 2 · 관리자는 조회만 두고 웹 쪽을 고치는 경로를 연다.** 지금 구조를 그대로 두고
  `admin/terms`는 「정본은 웹사이트」를 명시한 조회 화면으로 남긴다. 두 벌이 생기지 않지만
  대표님이 말한 「직접 조작」은 충족하지 못한다.

같은 문서를 두 벌 들면 한 벌이 낡고, 낡은 쪽을 사용자가 본다. 그래서 고르지 않고 올린다.
지금은 **안 2 상태 그대로** 두었고 화면 잠금과 사이드바 표시만 짝을 맞췄다.
