...
**terms** — **2026-09-16에 열렸다**(대표 지시 · 마이그레이션 0422). 쓰기 셋이 모두 400
「…앱 약관·동의 기록에 연결한 뒤 열려요」로 막혀 있었고, 그 문구가 적은 「연결한 뒤」가
그 작업이다. 본문이 `apps/web/src/subpages.ts`에서 표로 왔고 웹이 빌드할 때 읽어 그린다.
`READ_ONLY`에서도 빠졌다 — 화면의 `BACKEND_PENDING`과 함께 지웠다.
...
## 약관·방침의 「직접 조작」 — 2026-09-16에 대표님이 답하셨다

**끝난 항목이다.** 아래는 그때까지의 판단과 무엇이 그것을 뒤집었는지를 남긴다.

조사 중에는 판단이 필요하다고 적었고, 그때의 `main`이 이렇게 정해 두었다(PR #207 ·
`CLAUDE.md`) — 「약관과 개인정보처리방침의 정본은 웹사이트다. 본문은
`apps/web/src/subpages.ts` 하나이고, 앱·문서·관리자는 사본을 들지 않는다.」

그래서 관리자에 편집 화면을 만들지 않기로 했다. 같은 문서를 두 벌 들면 한 벌이 낡고,
낡은 쪽을 사용자가 본다. 그리고 마지막에 이렇게 적어 두었다 — 「대표님이 관리자에서
약관을 고치기를 원하신다면 그것은 정본을 웹에서 DB로 옮기는 일이고, `subpages.ts`의
본문 이전과 웹 빌드의 조문 조회가 함께 필요하다.」

**2026-09-16에 대표님이 그것을 원한다고 하셨다** — 「개인정보처리방침 이용약관 마케팅
약관도 동일하게 내가 수정가능하도록 하고」. 적어 둔 그대로 했다.

**뒤집힌 것은 「정본이 코드에 있다」뿐이다.** 「정본은 하나다」는 그대로다 — 본문은
표 하나이고, 웹이 그것을 읽어 그리고, 앱은 지금처럼 웹으로 내보낸다. 사본이 둘이 되는
위험(2026-09-09에 실제로 겪었다)은 막힌 채로 있다.

무엇이 생겼는지는 마이그레이션 `0422_legal_documents.sql`과 `apps/api/src/routes/legal.ts`에
적혀 있다. 판을 얼리는 것 · 시행일 · 국외 이전 절을 지울 때의 확인이 FAQ와 다른 점이다.
