...
2. 하이브리드 전환의 기술 방식부터 정한다(예: RN 쉘 + `react-native-webview`로
   `apps/web`의 대응 라우트를 로드, 로그인 세션을 웹뷰에 어떻게 넘길지, 네이티브
   탭바/네비게이션은 RN에 남기고 콘텐츠만 웹뷰인지 등) — 사용자가 "하이브리드"의
   구체 형태를 지정하지 않았으므로 착수 전 확인 권장.
3. 화면 단위로 전환하며 RN 유지 화면(WP-RPT-002, WP-NOTI-003)과 웹 미완성 화면
   (apps/web에 아직 없는 페이지)부터 순서를 정한다 — apps/web에 대응 페이지가
   없는 화면은 웹쪽부터 만들어야 웹뷰로 띄울 수 있다.
