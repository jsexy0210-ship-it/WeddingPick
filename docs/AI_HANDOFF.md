...
| 카카오 동의항목 · 만 14세 | **출생 연도로 판정 · 로그인 체크박스 없음** | 2026-09-09 사용자 결정. **필수** 출생 연도 · 프로필(닉네임·사진) / **안 씀** 연령대 · 생일 · 이름 · 성별 · 전화번호 · CI · 배송지. 선택 항목은 하나도 두지 않는다(2026-09-09). **경계 나이(올해−연도 == 14)는 막는다** — 생일을 안 받으므로 가릴 수단이 없고, 통과시키면 열세 살이 들어온다. **이름 · 성별 · 전화번호 · 연령대 · 생일은 받다가 뺐다** — 넣는 자리는 있는데 읽는 코드가 없었고, 카카오가 「불필요한 개인정보를 받으면 제재」라고 경고하는 자리다. 판정은 `routes/auth.ts`가 출생 연도로 한다 — 생일까지 오면 만 나이를 정확히 세고, 연도만이면 경계(올해−연도 == 14)는 `unknown`이다(통과도 차단도 아니다). 값은 판정만 뽑고 버린다. **로그인 화면의 「만 14세 이상이에요」 체크박스는 없앴다** — 스스로 누르는 선언이 아니라 받은 값으로 확인한다. **되돌리지 마라**: SPEC 3.5의 「체크박스 하나로 끝낸다」는 이 결정으로 무효다 |
...
| 카카오 동의항목 콘솔 | **필수 둘만 신청 · 나머지 전부 「사용 안 함」**(2026-09-09) | 신청 항목은 둘이다 — **필수** 출생 연도 · 프로필(닉네임·사진). 연령대 · 생일 · 전화번호 · 이름 · 성별 · CI는 「사용 안 함」이다. 코드도 같다 — `scopes`는 `openid · profile_nickname · profile_image · birthyear` 넷뿐이고, `/v2/user/me`는 `["kakao_account.birthyear"]`만 묻는다. **1차 심사 반려**(회원가입 절차 불명확 · 수집 항목 미기재 · 탈퇴 경로 누락)로 로그인 화면에 수집 항목 블록을 넣고 제출 이미지를 다시 만들었다 |
| 가입 화면 문구 되돌리기 | **심사 통과 후 버튼명을 다시 바꾼다**(2026-09-09 사용자 예고) | 지금 문구는 카카오 심사를 통과하기 위한 것이다 — 「웨딩픽 회원가입」 · 「가입하고 결혼 준비를 시작해 보세요.」 · **「카카오로 가입하기」**. 통과하면 버튼명을 되돌린다(사용자가 그때 정한다). **먼저 손대지 마라** — 통과 통지를 받기 전에는 그대로 둔다. 되돌릴 때 확인할 것 둘: ①심사는 이 화면을 근거로 났으므로 카카오가 다시 볼 수 있다 ②**수집 항목 안내 블록(`SIGNUP_PROFILE_FIELDS`)은 남긴다** — 「회원가입 화면 내 수집 항목·조건 기재 필수」가 반려 사유였다. 제목·설명을 함께 되돌릴지는 사용자에게 묻는다 |
