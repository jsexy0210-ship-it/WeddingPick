...
### 정책 2 — 홈 헤더 검색버튼 삭제
홈 탭(`apps/mobile/src/app/(tabs)/index.tsx`)의 `Header` 컴포넌트를 2026-09-04 main
기준(`2b7b0ca`)으로 재확인한 결과, 헤더에는 원래 검색 버튼이 없다 — 로그인 상태면
알림(🔔) 버튼만, 비회원 상태면 "로그인" 버튼만 있다(비회원 분기 자체는 정책 1과
얽혀 있어 아래 참고). **이 정책은 이미 만족돼 있다 — 코드 변경 불필요.** 검색은
하단 탭의 검색 탭(`(tabs)/search`)으로 계속 접근 가능.
(과거 이 문서가 "341번째 줄 부근에 검색 버튼이 있다"고 적었던 것은 이 정책이
확정되기 전 오래된 커밋을 기준으로 본 오기였다 — 실제로는 그 줄은 홈 안의
"취향 고르고 추천받기" CTA다.)

### 다음 작업 (미착수)
1. `react-native-webview` 설치 + 네이티브 래퍼 셸 구현
2. 웹 빌드 호스팅 방식 결정(`expo export -p web` 결과물을 어디에 올릴지)
3. 정책 1 실제 코드 반영(정책 2는 위처럼 이미 만족)
