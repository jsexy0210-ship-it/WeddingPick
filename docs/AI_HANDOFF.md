...
### 보고 경로 — 모든 보고는 MASTER를 거친다 (2026-09-09 사용자 지시)

사용자 지시: **「모든 알림 문제점 운영자 선택 여부 다 너를 통해 알림 받도록한다」**.
영역 세션은 사용자에게 직접 올리지 않는다. MASTER가 받아 정리해 한 번에 올린다.

보고 형식은 넷이다. 이 순서를 지킨다.

1. **한 줄 요약** — 무슨 일인지.
2. **무엇이 깨졌는지** — 지금 안 되는 것. 추측이 아니라 확인한 것만.
3. **선택지와 권고** — 두세 개와 권고안 하나. 비용·되돌리기 난이도를 함께.
4. **파일·줄 근거** — 어디를 보면 확인되는지.

- 사람만 할 수 있는 것(시크릿 값 · 외부 계정 · 계약 · 권한)은 MASTER가 사용자 조치 목록에 모은다.
- 돈이 드는 선택과 되돌리기 어려운 선택은 MASTER도 정하지 않는다. 사용자에게 올린다.
- 값(접속 문자열 · API 키 · 휴대폰 번호)은 보고에도 적지 않는다. 이름과 증상만 적는다.
