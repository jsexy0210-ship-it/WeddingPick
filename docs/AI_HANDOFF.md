...
| 카카오 동의항목 콘솔 | **전화번호까지 「사용 안 함」으로 설정 완료**(2026-09-09) | 신청 항목은 넷이다 — **필수** 출생 연도 · 프로필(닉네임·사진) / **선택** 연령대 · 생일. 전화번호는 받아서 넣는 자리는 있는데 읽는 코드가 없어 뺐다. 코드에서도 `phone_number` 스코프와 id_token의 `phone_number` 클레임을 지웠다 |
