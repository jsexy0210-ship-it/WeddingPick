...
- ✅ 남은 미구현 화면 카테고리(B2B 문의·커플 연결·우리웨딩·홈·기타) 재조사(Explore 서브에이전트) 후 실제로 가능한 것부터 구현:
  - **WP-OUR-013 예식 완료**: 이미 다 있었다 — `packages/domain/src/wedding-phase.ts`의 `COMPLETED_ACTIONS`(후기·미제보 결제내역·총지출 정리 문구)와 홈 화면의 `!showsPreparationFirst(stage.stage)` 분기가 예식 뒤 홈에 이미 뜨고 있었는데, 그 세 줄이 눌러도 아무 데도 안 가는 텍스트였다. `app/(tabs)/index.tsx`에 `goToCompletedAction()` 추가해 각각 `/pick`(후기 대상 고르기)·`/capture/payment/consent`(결제내역 등록)·지출내역 화면으로 연결. 새 화면도 새 API도 필요 없었다. **⚠️ 2026-09-02 main 병합 때 이 배선이 사라짐** — 다른 세션이 홈 화면 전체를 `디자인 확정본 웨딩픽 홈 C-1 상태`로 새로 설계(`features/home/state.ts`의 `homeView()`, `GuestHome`/`MemberHome` 분리)하면서 갈아엎었는데, 그 확정본의 5개 상태(`guest`/`taste`/`empty`/`picking`/`decided`)에는 예식 완료(post-wedding) 상태가 아예 없다. 병합 충돌에서 이 세션의 옛 코드 대신 새 확정본을 그대로 채택했다(도메인 레이어의 `COMPLETED_ACTIONS`는 그대로 남아 있고 테스트도 통과 — 화면에서만 안 씀). **판단 근거를 지어내지 않기 위해 다시 배선하지 않았다** — 이 확정본이 post-wedding을 의도적으로 뺀 건지 다음 단계로 미룬 건지 이 세션은 알 수 없어서 홈 설계 담당 세션에 확인을 요청했다.
  - **✅ 2026-09-02 사용자가 직접 정책으로 확정(더 물을 필요 없음)**: 초기 출시는 예식 당일까지만 지원한다. 예식 완료 단계 전용 기능은 사용자·업체 데이터가 어느 정도 모인 뒤 운영자가 안내하기 전까지 만들지 않는다 — 최신 통합정책서 D-4에 반영, `PROJECT_STATUS.md`에도 기록. 즉 새 홈 확정본이 post-wedding 상태를 안 가진 것이 맞는 설계였다. `goToCompletedAction()` 재배선은 하지 않는다(정책상 스코프 밖) — `COMPLETED_ACTIONS` 도메인 로직은 나중에 정책이 열리면 그대로 재사용할 수 있게 남겨둠.
...
- ✅ **모바일 앱을 실제로 처음 띄워서 브라우저로 확인** — 이번 세션 내내 `tsc`·jest로만 검증했지 실제 화면을 본 적이 없었다. 로컬 API 서버(`STORAGE_DRIVER=local`)와 마이그레이션 적용한 DB, `expo start --web`, Playwright(headless Chromium)로 최소 온보딩 → 홈 → 검색 → 박람회 → 웨딩 정보 → Pick 화면을 실제로 클릭해 넘기며 스크린샷 확인.
  - 과정에서 이 컨테이너에 `expo-crypto`(package.json에는 있으나 실제 설치가 안 돼 있었음)가 빠져 있어 웹 번들 자체가 안 뜨는 걸 발견 — `npm install`로 해소(락파일 변경 없음, 순수 컨테이너 상태 문제).
  - **부수 발견·수정: 검색 화면의 정렬 버튼이 "데이터 많은 순"이라고 뜨고 있었다** — v3.3이 사용자 화면에서 금지한 바로 그 낱말(`copy-rules.ts`의 예시 문구와 정확히 같음). 원인은 `packages/api-contract/src/vendors.ts`의 `VENDOR_SORT_LABEL.data` 상수가 정책이 `데이터`→`정보`로 바뀔 때 갱신되지 않고 남아 있던 것 — 화면(`search/index.tsx`)의 주석은 이미 `확인된 정보 많은 순`을 기대하고 있었는데 상수만 옛말이었다. `packages/domain`의 카피 스캐너들(`copy-rules.test.ts`·`user-copy.test.ts`)이 `packages/api-contract`는 훑지 않아 아무 테스트도 못 잡던 사각지대였다. `확인된 정보 많은 순`으로 고치고, 실제 화면에 반영된 것까지 스크린샷으로 재확인.
  - 검증: `packages/api-contract`·`apps/mobile`·`apps/api` `tsc --noEmit` 통과, `copy-rules.test.ts` 7/7 통과.
  - 부수 발견(고치지 않음, 스코프 밖): 비회원으로 `/pick` 접근 시 "로그인이 필요합니다"(합니다체) — 앱 전반의 해요체와 안 맞음. `apps/mobile/src/api/auth.ts`의 공용 에러 메시지라 여러 화면이 같이 씀 + `client.test.ts`가 이 정확한 문자열을 테스트로 고정해 둠 — 넓게 걸친 톤 통일 작업이라 이번 범위 밖으로 문서만 남김.
  - **이어서 업체 상세·InfoSheet도 실제로 확인하다가 같은 종류의 사각지대를 2건 더 찾아 고침**: `packages/domain/src/review.ts`의 `COLLECTING_LABEL`("데이터 수집 중" → "수집 중", 업체 상세의 "이용한 사람들의 경험" 섹션에 실제로 뜨고 있었음)과 `reidentification.ts`의 `NARROWED_NOT_ENOUGH`("이 조건은 아직 데이터가 모이는 중이에요" → "...정보가...", `apps/api/src/routes/vendors.ts`의 조건별 비교 응답에 실려 나가 업체 상세 화면에 그대로 뜸 — API 응답 텍스트라 도메인 카피 스캐너의 사정거리 밖이었다). 같이 발견한 `disclosure.ts`의 `CONDITION_NOT_ENOUGH`(같은 옛 문구, 지금은 아무 데서도 안 씀)도 나중에 잘못 다시 쓰이지 않도록 같이 고침.
  - 같은 방식으로 `pick-verification.ts`(결제 금지어 목록 자신이라 "결제 데이터"가 들어있는 게 맞음)·`pii-review.ts`·`advertising.ts`의 "데이터"는 확인해보니 전부 관리자·내부 전용이거나 아직 화면에 안 걸린 죽은 코드라 안 고침. `data-sources.ts`의 "지방행정 인허가 데이터"는 실제 정부 데이터셋의 공식 이름이라(`공공데이터`와 같은 이유로) 그대로 둠 — 바꾸면 출처 표시가 부정확해짐.
  - API 서버·웹 번들러 둘 다 `packages/domain` 변경을 반영하려면 **재시작해야** 한다는 걸 이번에 직접 겪었다(핫리로드 안 됨) — 도메인 상수를 고치고 화면에서 안 바뀌어 보이면 캐시가 아니라 서버 프로세스를 의심할 것.
**다음 세션 참고**: 관리자 화면 나머지(~19개 — 읽기 전용 14개 + 위험한 동작 5개), 박람회 후속(관리자 등록 UI 없음), WP-BIZ 006/007(B2B 계약·광고 스키마 설계 필요), WP-CPL(버전 관리 설계 필요), WP-HOME-006(랭킹 로직 제품 결정 필요), WP-OUR 일정 추가/지도 보기/취향 재선택(전부 이전 세션에서 차단 확인), 공용 "로그인이 필요합니다" 톤 통일(위 참조)이 남아 있다. **0052 마이그레이션 버그와 그 뒤에 생긴 0047/0052 스키마 중복 둘 다 고쳐져 main에 병합됐고 이 브랜치에도 반영됐다** — `apps/api` DB 연동 테스트와 production 마이그레이션 모두 정상 진행 가능(완전히 새 DB로 `apps/api` 566/566, 관리자 서버 실사용까지 재확인함). WP-OUR-013(예식 완료 CTA)은 홈 설계 세션 응답을 기다릴 필요 없이 사용자가 정책으로 직접 정리했다 — 초기 출시는 예식 당일까지만 지원, 재배선 안 함(위 참조, 최신 통합정책서 D-4).
...
11. ~~**[AI]** WP-OUR-013 예식 완료 CTA~~ — 2026-09-02 정책으로 종결. 초기 출시는 예식 당일까지만 지원하기로 확정(최신 통합정책서 D-4) — 재배선하지 않는다. 운영자가 사용자·업체 데이터 축적 후 예식 완료 단계 정책을 새로 안내하면 그때 `packages/domain/src/wedding-phase.ts`의 `COMPLETED_ACTIONS`를 재사용해 다시 배선
