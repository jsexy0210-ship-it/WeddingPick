...
**이미 적용된 마이그레이션의 이름을 바꾸지 않는다.** 번호가 겹치는 것 자체는 괜찮다 —
러너가 파일명 전체를 키로 쓰므로 겹친 번호도 둘 다 적용된다. 위험한 것은 이름을 바꾸는
쪽이고, 근거와 확인 방법은 [retention-policy.md](retention-policy.md)에 적었다.
