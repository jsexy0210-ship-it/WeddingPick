...
(async () => {
  ...
  function rootBoot({admin=false}={}){
    ...
    const api=load('apps/mobile/src/app/_layout.tsx',{
      ...
      'expo-splash-screen':{preventAutoHideAsync:nothing,hideAsync:nothing},'expo-router/head':{default:'Head'},'react-native':{Platform:{OS:'web'},View:'View',StyleSheet:{create:styles=>styles,absoluteFill:{}}},
    ...
    '@/features/navigation/depth-back':{dismissToOrReplace:nothing},
      ...
      '@/features/auth/signing-in-view':{SigningInView:'SigningIn',removeStaticAuthReturn:nothing},
      '@/features/loading/auth-progress':{beginAuthProgress:nothing},'@/api/client':{listAuthProviders:async()=>({providers:[]})},
