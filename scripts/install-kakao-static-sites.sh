...
if [ ! -f "$source_dir/web/privacy.html" ]; then
...
if grep -Eq '/var/www/weddingpick/releases/.*/web' "$CONF" 2>/dev/null; then
  static_is_live=true
fi
...
# marker 유실/불일치 상태에서 현재 served website를 먼저 rm -rf 하면, 이 시점에는
# rollback trap이 아직 없어서 복구할 수 없다.
if [ "$static_is_live" = true ]; then
  if [ -z "$live_sha" ]; then
    echo 'Static website is live but the live release marker is missing.' >&2
    exit 1
  fi
  live_root="/var/www/weddingpick/releases/$live_sha"
  if ! grep -Fq "root $live_root/web;" "$CONF" 2>/dev/null; then
...
trap - EXIT
echo "Kakao website 9443 enabled from release $release_sha; admin remains fixed on 443."
