...
- **2026-09-20 전달 디자인 정본**: `docs/design/source/2026-09-20/`의 ZIP 2개를 봉인하고 99개 활성 파일과 실제 고유 화면 ID 208개를 무결성 검사에 연결했다. 검색 Root Back 삭제와 삭제·통합 페이지 유지가 예외다. 208개 추적 대장은 만들었지만 구현 참조 미연결 30개와 390×844 시각 대조가 남아 전 화면 완료가 아니다.
- **현재 공개 앱웹은 PR462 기준**: 홈·검색을 모바일 폭으로 직접 대조한 결과 검색 Back 삭제와 즉시 결과는 반영됐지만 검색 제목·결과 헤더·카드 배치가 전달 정본과 달랐다. PR463·464의 main 배포는 API Object Storage CORS 단계에서 실패해 정적 후보 전환이 건너뛰어졌다.
- **PR465 동일 출처 문서 업로드**: PR466 배포에서 카카오 Object Storage preflight가 프로젝트 ID 없는 버킷 경로를 거부해 정적 전환이 중단됐다. 문서 원본은 인증된 동일 출처 API가 소유권·10MB 제한을 확인한 뒤 저장소에 기록하도록 바꾸고, 삭제된 CORS 검사 파일을 계속 호출하던 main·PR CI 누락도 제거했다. 후기 등 기존 서명 URL 계약은 유지한다.
