 ...
 *   - 알림 발송과 열람 — 이미 notification_deliveries가 담는다. 같은 것을 두 곳에
 *     적지 않는다
 *   - 접속 IP · 기기 식별자 — 처리방침의 「서비스 보안·운영」 항목이 따로 담는 값이다.
 *     원장은 행동만 받는다
 *   - 휴대폰 번호 · 카드번호 · 증빙 원문 · 녹음 — 스키마 CHECK가 막는다
 *
 * **글로만 적는다.** 처음에는 이것을 `ACTIVITY_OMITTED` 배열로 뒀는데, 읽는 코드가
 * 한 곳도 없으면서 사용자 문구 검사(`pick-language.test.ts`)에는 걸렸다 — 문서를
 * 코드인 척 두면 코드에 걸리는 규칙을 함께 진다. 같은 목록이 0280 마이그레이션과
 * `docs/activity-ledger.md`에도 있다.
 */
