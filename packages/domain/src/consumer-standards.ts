 ...
 *
 * **구간 조항에는 위쪽 끝(예식일에서 먼 쪽)을 넣는다.** 실제 계약서는
 * "149~60일 전 10%"처럼 구간으로 적는데 우리는 숫자 하나만 든다. 어느 쪽을
 * 넣어야 하는지가 답이 갈리는 문제로 보이지만 그렇지 않다.
 *
 * 기준 배상률은 날짜가 줄수록 오르고(0 → 10% → 20% → 35%) 조항의 비율은 구간
 * 안에서 일정하다. 그러면 둘의 차이는 **날짜가 가장 많이 남은 지점에서 가장
 * 크게 벌어진다.** 조항이 구간 어디에서든 기준보다 무겁다면 위쪽 끝에서도
 * 무겁다. 그래서 위쪽 끝 하나만 봐도 놓치는 경우가 없다.
 *
 * 아래쪽 끝을 넣으면 그 반대가 된다 — 149~60일 구간을 60으로 넣으면 기준과
 * 같아 보여, 100일 전에 취소하는 사람이 10%를 물게 되는 것을 알리지 못한다.
 */
