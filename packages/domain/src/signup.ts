...
/**
 * 만 14세 이상인지는 **카카오 연령대 하나로만** 확인한다. v3.13 §3.5.
 *
 * **자기 신고를 받지 않는다.** 예전에는 로그인 화면의 체크박스
 * («만 14세 이상이에요») 하나가 이 확인이었다. 핸드오프 v3.24가 그 체크박스를
 * 없앴고, 그 뒤로 확인은 카카오가 주는 연령대뿐이다.
 *
 * **확인 못 한 것은 통과가 아니다.** 체크박스가 사라진 뒤에도 서버는
 * 「연령대가 없으면 체크박스가 판정한다」 그대로였고, 판정할 체크박스가 없으니
 * 연령대를 못 받은 사람이 전부 무사통과했다 — 만 14세 미만 계정이 실제로
 * 가입됐다(2026-09-10 사용자 제보). 지금은 확인 못 하면 막는다.
 *
 * **생년월일을 받지 않는다.** 개인정보보호법은 만 14세 미만인지 확인하라고만
 * 정하고 방법을 지정하지 않는다. 연령대의 아래끝이면 충분하고, 생년월일보다
 * 적게 받는다.
 *
...
/** 만 14세 미만으로 확인됐을 때. WP-AUTH-010 이용 불가 안내가 이 문구를 쓴다. */
export const AGE_BLOCKED_NOTICE = `만 ${MINIMUM_AGE}세부터 이용할 수 있어요`;

/**
 * 나이를 **확인하지 못했을 때.** 미만이라고 말하지 않는다 — 확인이 안 된 것뿐이고,
 * 그 사람은 서른일 수도 있다. 사실만 말하고 무엇을 하면 되는지까지 적는다.
 *
 * 「~할 수 없어요」로 끝내지 않는다(용어 규칙) — 되는 길을 말한다.
 */
export const AGE_UNVERIFIED_NOTICE =
  '카카오에서 나이 확인 정보를 받지 못했어요. 카카오 계정 설정에서 연령대 제공에 동의한 뒤 다시 시도해주세요';
