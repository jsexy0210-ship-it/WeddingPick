 ...
 * 결정사 업체평가 — 체크리스트.
 *
 * **2026-09-24부터 새로 쓰이지 않는다.** 결정사 업종을 고르거나 수집하지 않으므로
 * (`RETIRED_VENDOR_CATEGORIES`) 이 방식으로 평가할 업체가 새로 생기지 않는다. 저장된
 * 체크리스트 응답(`review_checklist_answers`)과 DB 트리거가 이 규칙을 읽어 남겨 둔다.
 *
