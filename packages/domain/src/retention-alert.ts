 ...
 *
 * 파기 일정은 검증이 끝난 날로부터 센다. 그래서 파기 목록이 비어 있는 것은
 * "지울 것이 없다"일 수도 있고 "아직 셈이 시작되지 않았다"일 수도 있다.
 * 후자는 심사가 열려 있다는 뜻이고, 심사가 적체되면 개인정보가 그만큼 오래
 * 남는다.
 */
export const RETENTION_HELD_WARNING =
  '심사가 열려 있어 파기 일정이 서지 않은 원본이 있다. 파기 목록이 비어 있는 것은 지울 것이 없어서가 아니라 아직 셈이 시작되지 않아서일 수 있다.';
