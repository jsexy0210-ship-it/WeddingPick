 ...
 *
 * 보관 기간은 30일로 정해졌다(RETENTION_POLICY). 그런데 환경 설정에서 그것을
 * 꺼두면 retention_until이 비고, 파기 목록도 비고, 알림도 오지 않는다. 그건
 * "지울 것이 없다"가 아니라 "이 환경이 정책을 따르지 않고 있다"는 뜻이다 —
 * 원본은 무기한 쌓인다.
 */
export const RETENTION_UNSET_WARNING =
  '이 환경은 파기 일정을 만들지 않는다. 정책은 30일인데 설정이 그것을 끄고 있다. 알림이 오지 않는 것은 지울 것이 없어서가 아니라, 언제 지울지 계산하지 않기 때문이다. 원본은 계속 쌓인다.';
