 ...
 *
 * **2026-09-17에 대표님이 「둘을 합친다」로 정하셨다.** 위 결정에서 #230 판만
 * 남기며 main 판의 「녹취록은 만들지도, 남기지도 않아요」가 동의 화면에서 빠졌는데,
 * 그 줄을 되돌려 놓는다. 사실 자체는 이 파일 머리말과 개인정보처리방침 제1항
 * (「녹음 원본은 항목 추출이 끝나는 즉시 파기」)에도 있었지만 **동의를 누르는
 * 화면에는 없었다** — 방침을 열어 봐야 알 수 있는 것은 동의받은 것이 아니다.
 * 「본인이 참여한 대화만」은 동의 목록에서 빠진 채로 둔다 —
 ...
 * 동의받을 때 무엇을 말해야 하는가.
 *
 * 다섯 줄이다 — 읽어가는 것 · 남의 목소리 · 외부 서비스 · 녹취록 · (원본을 지운다 +
 * 서버에 남는 것)이 마지막 한 줄이다. 마지막 줄의 둘은 같은 이야기의 앞뒤라 줄을
 * 나누면 같은 말을 두 번 읽는 것이 된다.
 *
 * **「녹취록은 만들지도, 남기지도 않아요」는 따로 세운다.** 원본을 지우는 것과 다른
 * 말이기 때문이다 — 파일을 지워도 받아적은 글이 남으면 대화 내용은 그대로 남는다.
 * 읽는 사람이 가장 걱정하는 것이 그쪽이고, 마지막 줄은 그 답이 아니다.
 *
 ...
 */
export const VISIT_NOTE_AUDIO_CONSENT_VERSION = '2026-09-17';
