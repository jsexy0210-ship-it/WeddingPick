...
export const PARTNER_NOT_SHARED = [
  '원본 문서 파일 자체 — 올린 사람만 가져요',
  '상대방의 계정 정보 (이메일, 로그인 수단)',
  '자료 확인 신청에 낸 증빙 — 낸 사람만 쓸 수 있어요',
] as const;

/** 연결을 끊으면 어떻게 되는지. 끊기 전에 알려준다. */
export const PARTNER_UNLINK_EFFECTS = [
  '상대방은 이 웨딩의 금액과 비교 결과를 더 볼 수 없어요.',
  '각자 올린 원본 문서는 올린 사람에게 그대로 남아요.',
  '이미 확인을 마친 자료는 가격 비교에 그대로 쓰여요 — 연결과 무관해요.',
] as const;
...
export const INVITE_STATE_MESSAGE: Record<InviteState, string> = {
  usable: '',
  expired: '초대 기한이 지났어요. 초대한 분께 다시 보내달라고 해주세요.',
  revoked: '취소된 초대예요. 초대한 분께 다시 보내달라고 해주세요.',
  accepted: '이미 사용된 초대예요.',
  already_linked: '이미 배우자가 연결되어 있어요. 한 웨딩에는 두 사람까지예요.',
};
