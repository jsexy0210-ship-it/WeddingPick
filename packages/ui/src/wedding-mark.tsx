 ...
 *
 * 확정본이다. `spec/tokens.json`의 `symbol` 블록과 CLAUDE.md §2가 원본이고,
 * 아래 두 경로·viewBox·획 두께를 어떤 이유로도 바꾸지 않는다.
 * 앱 아이콘, 스플래시, 시작 화면, Pick 탭, Pick 완료, 웹 GNB, 관리자가 같은 마크를 쓴다.
 *
 * 좌표는 24 칸 격자다. 한때 64 칸으로 다시 그린 판이 앱에만 남아 있었는데,
 * 그쪽은 체크가 하트 폭의 절반(24/48)을 차지해 확정본(6.6/17)보다 눈에 띄게 컸다.
 * 웹·`product-symbol`은 이미 확정본을 쓰고 있어서 같은 서비스가 두 마크를 쓰고 있었다.
 */
export const MARK_VIEWBOX = 24;

/** 하트 윤곽. `spec/tokens.json` `symbol.paths[0]`. */
