 ...
 * **24다**(2026-09-17 대표 지시 — 새 패키지를 따른다). 근거는
 * `docs/design/handoff/README.md`의 「좌우 Gutter 24px 고정. 전 화면 예외 없음」.
 *
 * **이 값이 20↔24로 세 번 뒤집혔다.** 그 이력을 적어 두는 이유는 다음에 또 재는
 ...
 * 8개가 20이던 때 좌우 끝선이 4px씩 어긋났고, 대표님이 처음 짚으신 자리가 그것이다.
 * 그래서 화면이 저마다 숫자를 적지 않고 이 한 줄을 본다.
 */
export const Layout = {
  /** 화면 좌우 거터. 바텀시트 내부도 같다. spec/tokens.json spacing.gutter. */
  gutter: 24,
  /** `gutter`와 같은 값. 규격서를 그대로 옮긴 화면들이 이 이름을 쓴다. 둘은 늘 같다. */
  pageX: 24,
