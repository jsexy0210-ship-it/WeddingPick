 ...
 * 피그마 12 화면 중 **10 화면**이 20이다(`docs/figma-spec/*.txt`의 `pad … 20 … 20`).
 ...
 *
 * **처음 이 주석은 「12 화면 전부가 20」이라고 적었고 거짓이었다.** 스크롤 화면의
 * 안쪽 섹션만 보고 열둘로 일반화한 것이다. 로그인·온보딩은 스크롤이 없는 한 장짜리라
 * **바깥 셸이 좌우 24를 직접 든다**(`shellGutter`). 그 거짓 근거 때문에 두 화면이
 * 양쪽 4px씩 좁아진 채로 갔다 — 재지 않고 일반화하면 이렇게 된다.
 */
export const Layout = {
  ...
  pageX: 20,
  /**
   * 로그인 · 온보딩의 좌우 여백. **이 둘만 쓴다.**
   *
   * 스크롤이 없는 한 장짜리 화면이라 바깥 셸이 여백을 직접 든다 —
   * `docs/figma-spec/login.txt` 「div 430×932 pad 64 24 32 24」 ·
   * `onboarding.txt` 「div 430×932 pad 32 24 32 24」. 두 화면 다 안쪽 블록이
   * 382 폭이고 430 − 24 − 24 = 382로 떨어진다.
   *
   * 나머지 10 화면은 셸의 좌우가 0이고 안쪽 섹션이 `gutter` 20을 든다.
   * spec/tokens.json spacing.shellGutter.
   */
  shellGutter: 24,
