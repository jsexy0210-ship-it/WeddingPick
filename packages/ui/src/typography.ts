 ...
 * sub 16 · caption 14 · micro 13 — 을 핸드오프 이름(t1~t7)으로 든다. **17 · 19 · 22px은 금지다.**
 *
 * **15는 2026-09-15에 금지에서 풀렸다.** 그때까지 이 줄은 「15 · 17 · 19 · 22px은 금지」라고
 * 적었는데 아래 `f15`가 있고 네 곳이 쓰고 있었다 — 규칙과 코드가 어긋나 있었던 것이다.
 * 어느 쪽이 낡았는지 피그마를 재서 갈랐다: **로그인의 카카오 CTA가 `15/700 · lh 23`이다**
 * (`docs/figma-spec/login.txt` «button 382×56 · bg #FEE500»). 피그마에 15가 있으므로
 * **코드가 맞고 규칙이 낡았다.** 17 · 19 · 22는 실측 근거가 없어 그대로 금지다.
 *
...
export const LineHeight = {
  /*
   * 32/42 — SEED `$line-height.t12`.
   *
   * **2026-09-15까지 43이었다.** 나머지 일곱(t2~t7 · micro · badge)은 SEED와 값이 그대로
   * 같은데 이 하나만 1px 어긋나 있었다. 피그마에서 32px 글자는 `vendor-1.txt`의 업체명
   * 하나뿐이고 그쪽은 lh 40이면서 **역할이 다르다**(히어로 제목 · `f32` 사다리 몫).
   * 이 자리(display — 홈 Hero · 큰 금액)를 받치는 피그마 실측값이 없어서 SEED로 맞췄다.
   */
  t1: 42,
  t2: 35,
  t3: 32,
