...
--
-- **탭과 카테고리 둘 다 코드에 있던 것을 옮겨 온 값이다.**
--
-- 탭 셋은 `WEDDING_FEED_GROUPS`(domain)에 있던 그대로다 — 이름도, 어느 카테고리가
-- 어느 탭에 드는지도 바꾸지 않았다. 카테고리 열셋은 `WEDDING_FEED_TOPICS` 열여덟
-- 항목의 `category_label`을 중복 제거한 값이고 **글자 하나 바꾸지 않고** 넣는다.
-- 그래야 이미 쌓인 글이 전부 그대로 붙고, 마이그레이션이 돌아도 화면이 그대로다.
--
-- **이 문단은 처음에 틀리게 적혀 있었다. 무엇이 틀렸는지 남긴다.**
-- 원래 「탭은 옮겨 온 것이 아니다 — main(f59104ba)에는 그룹 상수가 없다」라고 적혀
-- 있었다. 그 문장은 **작업 세션의 클론이 뒤처져 있어서 나온 것이다** —
-- `WEDDING_FEED_GROUPS`는 같은 날 `33c0aebd`로 이미 들어와 있었고, 여기 넣는 씨앗값이
-- 바로 그 상수에서 왔다. 값을 처음 세운 것이 아니라 **옮긴 것**이다.
--
-- 지우지 않고 적어 두는 이유는 CLAUDE.md가 못 박은 자리이기 때문이다 — 여기 적힌
-- 거짓 근거는 다음 세션 전부가 확정 사양으로 읽고 그 위에 쌓는다. **오래된 클론을
-- 근거로 「main에는 없다」고 적지 않는다.** 적기 전에 `git fetch`부터 한다.
--
