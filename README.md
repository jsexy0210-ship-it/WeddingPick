...
Phase 2에서는 업체 검색(A-16)과 업체 상세(A-17)가 동작한다. 별점도 후기도 순위도 없다 — 보여주는 것은 업체가 있다는 사실, 분류와 지역, 확인된 계약이 몇 건 모였는지, 그리고 표본이 충분한 상품의 실제 계약 중앙값뿐이다. 자료가 없으면 없다고 적는다. 최대 3곳까지 나란히 견줄 수 있고, 그 결과에는 "금액만으로는 비교하기 어렵다"는 말이 늘 함께 나간다 — 표만 그리고 그 말을 빼면 우리가 만든 표가 오해를 부추긴다. 플래너 탐색은 아직 만들지 않았다.
...
- **플래너 검색 공개 여부** — 개인 이름을 목록으로 공개하는 것은 업체를 공개하는 것과 다른 문제다. 법률검토 결과가 필요하다.
