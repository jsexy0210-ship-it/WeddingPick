...
[개발 순서](docs/05-product-spec.md#6-개발-순서-제안) 1단계(레포 구조·스캐폴드·CI) 완료, 3단계 진행 중. 홈(A-03)과 촬영·문서 확인(A-04·A-05) 화면이 동작한다. 분석 결과가 없는 동안 홈은 빈 상태와 서비스 흐름 안내를 보여준다 — 가짜 견적·가격은 채우지 않는다. 문서는 아직 기기 안에만 있고, 서버 업로드와 AI 분석은 원본 문서 처리에 대한 법률 검토 이후다.
