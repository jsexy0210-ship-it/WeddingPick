...
Phase 2에서는 업체 검색(A-16)과 업체 상세(A-17)가 동작한다. 별점도 후기도 순위도 없다 — 보여주는 것은 업체가 있다는 사실, 분류와 지역, 확인된 계약이 몇 건 모였는지, 그리고 표본이 충분한 상품의 실제 계약 중앙값뿐이다. 자료가 없으면 없다고 적는다. 최대 3곳까지 나란히 견줄 수 있고, 그 결과에는 "금액만으로는 비교하기 어렵다"는 말이 늘 함께 나간다 — 표만 그리고 그 말을 빼면 우리가 만든 표가 오해를 부추긴다. 플래너도 찾을 수 있지만 **공개 근거가 있는 사람만** 나온다 — 견적서에서 읽어낸 이름은 검색 목록에 오르지 않도록 스키마가 막는다. 화면에는 왜 검색에 나오는지와 원하지 않을 때 어떻게 하는지가 늘 함께 나간다.
...
자세한 것은 [제품·기술 명세 13번 열린 질문](docs/05-product-spec.md).
