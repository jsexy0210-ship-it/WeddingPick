...
const PRIVACY_SECTIONS: PrivacySection[] = [
  {
    ...
    rows: [
      ...
      ['소셜 프로필 기본값(선택)', '소셜 제공자가 동의 범위에 따라 전달한 이름, 이메일, 닉네임, 프로필 이미지, 성별, 휴대전화번호. 연령대·생년월일·출생연도는 프로필 저장 항목에서 제외', '계정 연결 해제, 직접 삭제 또는 탈퇴 시까지. 원본 응답 전체는 장기 보관하지 않음'],
  ...
  },
  {
    ...
    lead: '국외 이전에 대한 문의와 처리정지·동의철회 요청은 고객센터로 접수할 수 있습니다. 자료 분석을 위한 이전을 원하지 않는 경우 자료를 제출하지 않을 수 있으며, 푸시 알림은 기기 알림 설정에서 끌 수 있습니다. 자료 분석이나 푸시 알림을 이용하지 않는다는 이유만으로 기본 서비스 이용을 제한하지 않습니다. 서비스 운영·정보 저장을 위한 이전 중지를 요청하는 경우에는 해당 처리의 필요성과 법적 근거, 계정 유지에 미치는 영향을 구분하여 안내합니다.',
  },
  ...
  { t: '8. 만 14세 미만 아동', l: ["웨딩픽은 만 14세 미만의 회원가입을 받지 않습니다. 소셜 제공자가 전달하는 연령대와 가입 단계의 연령 확인을 이용하여 가입 자격을 판정하고, 판정에 사용한 연령대·생년월일·출생연도는 프로필에 새로 저장하지 않습니다. 만 14세 이상 확인 결과와 가입·동의 이력을 보관합니다."] },
