import { termsArticles, privacySections, legalDocument, privacyDocument } from '@weddingpick/domain';
...
export function renderTermsPage(): string {
  return subDocument({
    ...
    body: liveLegalDocument('terms', legalDocument(termsArticles(CONTACT_EMAIL))),
  });
}
...
export function renderPrivacyPage(): string {
  return subDocument({
    ...
    body: liveLegalDocument('privacy', privacyDocument(privacySections(CONTACT_EMAIL))),
  });
}

function liveLegalDocument(doc: 'terms' | 'privacy', fallback: string): string {
  const origin = apiBase();
  if (!origin) return fallback;
  const url = JSON.stringify(`${origin}/v1/legal/${doc}`).replace(/</g, '\\u003c');
  return `<div id="published-legal" hidden>${fallback}</div><p id="legal-publication" class="sp-support-note">최신 문서를 불러오는 중이에요.</p><button id="legal-retry" type="button" hidden>다시 불러오기</button><noscript><p>최신 약관을 확인하려면 자바스크립트를 켜주세요.</p></noscript>
<script>(function(){var content=document.getElementById('published-legal'),status=document.getElementById('legal-publication'),retry=document.getElementById('legal-retry');function load(){content.hidden=true;retry.hidden=true;status.textContent='최신 문서를 불러오는 중이에요.';fetch(${url},{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('문서를 불러오지 못했어요');return r.json()}).then(function(r){if(r.document){content.innerHTML=r.document.html;status.textContent='공개 버전 '+r.document.version}else{status.textContent=''}content.hidden=false}).catch(function(){content.hidden=true;status.textContent='최신 문서를 불러오지 못했어요.';retry.hidden=false})}retry.addEventListener('click',load);load()})()</script>`;
}
