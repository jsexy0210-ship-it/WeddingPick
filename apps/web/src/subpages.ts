...
const PRIVACY_SECTIONS: PrivacySection[] = [
  {
    ...
    rows: [
      ...
      ['상담기록 정리(선택)', '이용자가 올린 상담 녹음 파일, 그 녹음에서 읽어낸 업체명·상담일·금액·상품 구성·계약 조건·상담 요약, 금액 항목의 근거가 된 짧은 인용(40자 이내). 녹음에는 이용자 외 다른 사람의 목소리가 함께 담길 수 있습니다', '녹음 원본은 읽어내기가 끝나면 바로 삭제하고, 끝나지 않은 경우에도 업로드 시점부터 24시간 안에 삭제합니다. 읽어낸 항목은 직접 삭제 또는 탈퇴 시까지 보관합니다. 녹취록은 만들지 않습니다'],
  ...
  { t: '3. 개인정보의 제3자 제공', l: ["회사는 정보주체의 동의 또는 법령상 근거가 있는 경우에만 개인정보를 제3자에게 제공합니다. 업체 상담·견적·예약 기능이나 보상 발송을 위해 제공이 필요한 경우 제공받는 자, 목적, 항목, 보유기간, 동의 거부권을 해당 기능 사용 전에 별도로 알립니다."] },
  {
    ...
    rows: [
      ['서비스 운영·호스팅', '서비스 서버·정적 웹 운영', '(주)카카오엔터프라이즈(KakaoCloud) · 서비스 이용 또는 위탁계약 종료 시까지'],
      ['정보 저장소 운영', '서비스 정보의 저장과 조회', 'Neon, Inc. · 위탁계약 종료 또는 서비스 종료 시까지'],
      ['클라우드·객체 저장', '신규 업로드 원본의 시한부 저장', '(주)카카오엔터프라이즈(KakaoCloud) · 이 처리방침이 정한 보유기간까지'],
      ['기존 객체 저장', '전환 전 업로드 원본의 잔존 보관', '네이버클라우드 주식회사 · 기존 보관분이 있는 경우 이 처리방침이 정한 보유기간까지'],
      ['문자인식·영상 분석', '업로드한 자료에서 필요한 항목 추출·가림·검증', 'Google LLC · 회사는 업로드 원본을 제2항의 기준으로 삭제하며, 수탁자의 입력·출력 보존기준은 제5항을 따릅니다'],
      ['음성 인식·상담내용 분석', '이용자가 올린 상담 녹음에서 업체명·금액·조건 등 필요한 항목 추출', 'Google LLC · 회사는 녹음 원본을 읽어내기 직후 삭제하며, 수탁자의 입력·출력 보존기준은 제5항을 따릅니다'],
  ...
  },
  {
    ...
    rows: [
      ...
      [
        ...
        '미국 · 업로드한 자료와 상담 녹음에서 업체명·금액·계약 조건 등 필요한 항목을 읽어내기 위해',
        '이용자가 올린 자료의 이미지 또는 PDF 파일 전체와 상담 녹음 파일 전체(업체명, 금액, 예식일, 상담 내용, 이름과 연락처가 함께 적히거나 담겨 있을 수 있고, 녹음에는 이용자 외 다른 사람의 목소리가 함께 담길 수 있습니다). 이용자가 자료 또는 녹음을 올려 분석이 시작되는 시점에, 암호화된 통신구간을 통해 사업자의 응용프로그램 인터페이스로 전송합니다. 보존기준은 사업자가 공개한 정책을 따르며, 회사 저장소의 원본 삭제 일정과는 별도입니다. 회사 저장소의 녹음 원본은 항목 추출이 끝나는 즉시 파기하며, 업로드 자료 원본의 삭제 일정은 제2항에 따릅니다. 회사는 이 이전을 유료 API 경로로만 수행하고 모델 학습에 이용되지 않는 조건으로 처리합니다.',
      ],
      [
    ...
    lead: '이 표의 「국가」는 개인정보가 처리·보관되는 서버 리전과 전송망의 범위를 뜻하며, 이전받는 자의 사업자 소재지와 다를 수 있습니다. Neon의 정보 저장소는 싱가포르 리전을 사용합니다. KakaoCloud의 서비스 서버·정적 웹·Object Storage는 국내 인프라에서 운영되므로 국외 이전 표에 포함하지 않습니다. 국외 이전에 대한 문의와 처리정지·동의철회 요청은 고객센터로 접수할 수 있습니다. 자료 분석과 상담 녹음 정리를 위한 이전을 원하지 않는 경우 자료나 녹음을 제출하지 않을 수 있으며, 푸시 알림은 기기 알림 설정에서 끌 수 있습니다. 자료 분석, 상담 녹음 정리, 푸시 알림을 이용하지 않는다는 이유만으로 기본 서비스 이용을 제한하지 않습니다. 정보 저장을 위한 이전 중지를 요청하는 경우에는 해당 처리의 필요성과 법적 근거, 계정 유지에 미치는 영향을 구분하여 안내합니다.',
  },
  ...
  { t: '10. 안전성 확보조치', l: ["접근권한 최소화와 역할 분리", "전송·저장 구간 암호화", "접속기록 보관과 위변조 방지", "취약점 점검과 침해사고 대응", "수탁자 관리·감독", "원본 이미지 접근 제한과 자동 삭제 검증", "상담 녹음 원본의 접근 제한과 읽어내기 직후 삭제 검증"] },
