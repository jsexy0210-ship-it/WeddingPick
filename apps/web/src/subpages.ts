...
const PRIVACY_SECTIONS: PrivacySection[] = [
  {
    ...
    rows: [
      ...
      ['상담 녹음에서 방문노트 정리', '음성(이용자가 올린 상담 녹음 파일). 업체 이름, 방문한 날, 그 자리에서 들은 금액, 메모가 담기며 이용자 외 다른 사람의 목소리가 함께 담길 수 있습니다. 녹음에서 읽어낸 방문노트 항목', '녹음 원본은 방문노트 항목 추출이 끝나는 즉시 파기합니다. 읽어낸 방문노트 항목은 직접 삭제 또는 탈퇴 시까지 보관합니다.'],
  ...
  { t: '3. 개인정보의 제3자 제공', l: ["회사는 정보주체의 동의 또는 법령상 근거가 있는 경우에만 개인정보를 제3자에게 제공합니다. 업체 상담·견적·예약 기능이나 보상 발송을 위해 제공이 필요한 경우 제공받는 자, 목적, 항목, 보유기간, 동의 거부권을 해당 기능 사용 전에 별도로 알립니다."] },
  {
    ...
    rows: [
      ...
      ['문자인식·영상 분석', '업로드한 자료에서 필요한 항목 추출·가림·검증', 'Google(Gemini API) · 법인명과 수탁자 보유기준은 확인 필요 — 2026-09-14 기준 공개 정책 원문을 확인하지 못했습니다(제5항 참조)'],
      ['상담 녹음 정리', '이용자가 올린 상담 녹음에서 방문노트 항목 추출', 'Google(Gemini API) · 회사는 항목 추출이 끝나는 즉시 녹음 원본을 파기합니다. 법인명과 수탁자 보유기준은 확인 필요 — 2026-09-14 기준 공개 정책 원문을 확인하지 못했습니다(제5항 참조)'],
  ...
  },
  {
    ...
    rows: [
      [
        'Google(Gemini API) · 법인명·연락처 확인 필요',
        '확인 필요(처리·보관 국가) · 업로드한 자료와 상담 녹음에서 금액·업체명 등 필요한 항목을 읽어내기 위해',
        '이용자가 올린 자료의 이미지 또는 PDF 파일 전체와 상담 녹음 파일 전체(업체명, 금액, 예식일, 이름, 연락처가 함께 적히거나 담겨 있을 수 있고, 녹음에는 이용자 외 다른 사람의 목소리가 함께 담길 수 있습니다). 이용자가 자료 또는 녹음을 올려 분석이 시작되는 시점에, 암호화된 통신구간을 통해 사업자의 응용프로그램 인터페이스로 전송합니다. 이전받는 자의 보유기간과 그 예외는 확인 필요 — 2026-09-14 기준 공개 정책 원문을 확인하지 못했습니다. 이전받는 자가 보유하는 사본은 회사 저장소의 원본 삭제 일정과는 별도입니다. 회사 저장소의 녹음 원본은 항목 추출이 끝나는 즉시 파기하며, 업로드 자료 원본의 삭제 일정은 제2항에 따릅니다.',
      ],
      [
        'Render Services, Inc. · privacy@render.com',
        '싱가포르(운영 API 및 주기 작업) · 전 세계(정적 웹 전송망, 사업자 소재지: 미국) · 서비스 서버 운영',
    ...
    lead: '이 표의 「국가」는 개인정보가 처리·보관되는 서버 리전과 전송망의 범위를 뜻하며, 이전받는 자의 사업자 소재지와 다를 수 있습니다. 이 처리방침 시행일부터 Render의 운영 API와 Neon의 정보 저장소는 싱가포르 리전을 사용합니다. 주기 작업(원본 파기 정리·알림 발송·자료 분석)은 같은 싱가포르 운영 API 안에서 처리하며, Render의 미국 리전 서버는 사용하지 않습니다. 정적 웹은 전 세계 전송망에서 제공됩니다. 국외 이전에 대한 문의와 처리정지·동의철회 요청은 고객센터로 접수할 수 있습니다. 자료 분석과 상담 녹음 정리를 위한 이전을 원하지 않는 경우 자료나 녹음을 제출하지 않을 수 있으며, 푸시 알림은 기기 알림 설정에서 끌 수 있습니다. 자료 분석, 상담 녹음 정리, 푸시 알림을 이용하지 않는다는 이유만으로 기본 서비스 이용을 제한하지 않습니다. 서비스 운영·정보 저장을 위한 이전 중지를 요청하는 경우에는 해당 처리의 필요성과 법적 근거, 계정 유지에 미치는 영향을 구분하여 안내합니다.',
  },
