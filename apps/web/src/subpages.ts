  ...
  },
  {
    ...
    rows: [
      [
        ...
        '싱가포르(운영 API 및 백그라운드 처리) · 전 세계(정적 웹 전송망, 사업자 소재지: 미국) · 서비스 서버 운영',
    ...
    lead: '이 표의 「국가」는 개인정보가 처리·보관되는 서버 리전과 전송망의 범위를 뜻하며, 이전받는 자의 사업자 소재지와 다를 수 있습니다. 이 처리방침 시행일부터 Render의 운영 API와 Neon의 정보 저장소는 싱가포르 리전을 사용하며, 백그라운드 처리도 같은 운영 API 안에서 이루어져 싱가포르 리전에 있습니다. 정적 웹은 전 세계 전송망에서 제공됩니다. 국외 이전에 대한 문의와 처리정지·동의철회 요청은 고객센터로 접수할 수 있습니다. 자료 분석과 상담 녹음 정리를 위한 이전을 원하지 않는 경우 자료나 녹음을 제출하지 않을 수 있으며, 푸시 알림은 기기 알림 설정에서 끌 수 있습니다. 자료 분석, 상담 녹음 정리, 푸시 알림을 이용하지 않는다는 이유만으로 기본 서비스 이용을 제한하지 않습니다. 서비스 운영·정보 저장을 위한 이전 중지를 요청하는 경우에는 해당 처리의 필요성과 법적 근거, 계정 유지에 미치는 영향을 구분하여 안내합니다.',
  },
