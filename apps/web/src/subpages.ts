...
const PRIVACY_SECTIONS: PrivacySection[] = [
  ...
  },
  {
    ...
    lead: '이 표의 「국가」는 개인정보가 실제로 처리·보관되는 서버 리전을 뜻하며, 이전받는 자의 사업자 소재지와 다를 수 있습니다. 정보 저장소를 운영하는 Neon, Inc.는 미국 사업자이나 서비스 정보는 싱가포르 리전에 보관합니다. 국외 이전에 대한 문의와 처리정지·동의철회 요청은 고객센터로 접수할 수 있습니다. 자료 분석을 위한 이전을 원하지 않는 경우 자료를 제출하지 않을 수 있으며, 푸시 알림은 기기 알림 설정에서 끌 수 있습니다. 자료 분석이나 푸시 알림을 이용하지 않는다는 이유만으로 기본 서비스 이용을 제한하지 않습니다. 서비스 운영·정보 저장을 위한 이전 중지를 요청하는 경우에는 해당 처리의 필요성과 법적 근거, 계정 유지에 미치는 영향을 구분하여 안내합니다.',
  },
