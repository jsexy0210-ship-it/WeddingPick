...
export function renderIntroPage(): string {
  const body = `<div style="padding:0 64px 72px;display:flex;flex-direction:column;gap:0">
    ...
    ${introSteps('이렇게 씁니다', [
      ...
      { no: '4', title: '둘이 같이 골라요', body: '각자 Pick하고 겹치는 곳부터 좁혀나가요. 고르면 우리웨딩에 바로 반영돼요.', meta: 'Pick' },
    ])}
...
const FAQ_ROWS = [
  ...
  { q: '회원가입 없이 사용할 수 있나요?', a: '확인된 제보는 로그인 없이 볼 수 있어요. Pick, 우리웨딩, Pick 인증은 가입 후 이용하세요.' },
...
const PRIVACY_SECTIONS: PrivacySection[] = [
  ...
  { t: '3. 개인정보의 제3자 제공', l: ["회사는 정보주체의 동의 또는 법령상 근거가 있는 경우에만 개인정보를 제3자에게 제공합니다. 업체 상담·견적·예약 기능이나 보상 발송을 위해 제공이 필요한 경우 제공받는 자, 목적, 항목, 보유기간, 동의 거부권을 해당 기능 사용 전에 별도로 알립니다."] },
  {
    ...
    rows: [
      ['클라우드·객체 저장', '서비스 데이터 및 Pick 인증 원본의 시한부 저장', '수탁자가 정해지는 경우 서비스 내 처리방침에 공개'],
      ['문자인식·영상 분석', 'Pick 인증 자료에서 필요한 항목 추출·가림·검증', '수탁자가 정해지는 경우 서비스 내 처리방침에 공개'],
      ['인증', '카카오·네이버·구글·애플 소셜 인증', '각 제공자의 개인정보 처리방침 및 이용자 선택에 따름'],
  ...
  { t: '5. 개인정보의 국외 이전', l: ["해외 리전 또는 해외 사업자가 개인정보를 처리하는 경우 이전 국가, 받는 자, 목적, 항목, 시기·방법, 보유기간 및 이전 거부 방법을 공개합니다. 국외 이전이 발생하는 경우 이전 국가, 이전받는 자, 목적, 항목, 시기·방법, 보유기간과 거부 방법을 서비스 내 개인정보 처리방침에 공개합니다."] },
...
export function renderPrivacyPage(): string {
  return subDocument({
    ...
    activePath: null,
    titleBand: titleBand('홈 · 개인정보처리방침', '웨딩픽 개인정보 처리방침', '웨딩픽은 개인정보 보호법 등 관계 법령을 준수하며, 서비스에 필요한 최소한의 개인정보만 처리합니다.'),
