...
`ORIGINAL_RETENTION_DAYS`를 설정해야 파기 일정이 선다. **비워두면 일정이 없고**,
워커가 켜질 때 그 사실을 경고한다. 서비스정책서의 미확정 항목이라 기본값을 두지 않았다 —
임의의 숫자로 남의 계약서를 지우거나 무기한 보관하는 쪽을 조용히 고르지 않으려는 것이다.

비어 있는 동안은 알림도 오지 않는다. 그건 "지울 것이 없다"가 아니라 "언제 지울지
정하지 않았다"는 뜻이다.
