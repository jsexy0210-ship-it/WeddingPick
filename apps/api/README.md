...
**업로드일로부터 30일** (2026-08-28 확정). 값은 도메인의 `RETENTION_POLICY.originalDays`
한 곳에 있고, `ORIGINAL_RETENTION_DAYS`는 그것을 덮어쓴다. 환경변수를 비워두면 정책값이
쓰인다 — 빠뜨린 환경이 무기한 보관으로 떨어지지 않게 한 것이다.

`ORIGINAL_RETENTION_DAYS=0`처럼 꺼두면 파기 일정이 서지 않고, 워커가 켜질 때 경고한다.
그건 "지울 것이 없다"가 아니라 "이 환경이 정책을 따르지 않는다"는 뜻이다.
...
- **보관 기간 확정** — 자동삭제 작업은 있지만 `ORIGINAL_RETENTION_DAYS`가 없으면 돌지 않는다.
  법률 자문 후 일수를 정해야 한다
