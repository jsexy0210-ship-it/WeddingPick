...
앱이 출시되기 전에는 실제 견적서·계약서가 들어올 경로가 없다. 파이프라인이 진짜
문서에서 무엇을 뽑는지 보려면 파일에서 바로 넣는 길이 있어야 한다.

```
ANTHROPIC_API_KEY=... npm run analyze --workspace @weddingpick/api -- \
  --file 계약서.pdf --dry-run
```

`--dry-run`은 분석만 하고 저장하지 않는다. **남의 계약서로 시험할 때는 이걸 쓴다**
— 실험이 데이터가 되어 남으면 안 된다. 뽑아낸 개인정보는 값이 아니라 종류만
보여주므로, 그 규칙이 실제로 지켜졌는지 눈으로 확인할 수 있다.

`--dry-run` 없이 넣으면 다른 문서와 똑같은 취급을 받는다. 등급은 L0에서 시작하고,
사용자 확인과 개인정보 재검토를 거치기 전에는 어떤 비교에도 들어가지 않는다.

받는 형식: pdf, jpg, png, webp, gif.
