...
describeWithDb("결제인증", () => {
  ...
  it("가맹점 이름으로 업체를 찾는다", async () => {
    ...
    expect(response.json<{ matchedVendorId: string }>().matchedVendorId).toBe(
      vendorId,
    );
  });
  ...
  it("업체를 못 찾아도 버리지 않는다", async () => {
    ...
    expect(response.statusCode).toBe(201);
    expect(
      response.json<{ matchedVendorId: null }>().matchedVendorId,
    ).toBeNull();
    // 무엇을 해야 하는지 알려준다. 그냥 실패로 두면 사용자가 할 수 있는 게 없다.
    expect(
      response.json<{ unmatchedNote: string }>().unmatchedNote,
    ).toBeTruthy();
  ...
  it("카드번호를 보낼 자리가 없다", async () => {
    ...
    expect(stored.rows[0]!.masked_identifiers).toEqual([
      "card_number",
      "approval_number",
    ]);
  ...
  it("문서 등급을 올리지 않는다", async () => {
     ...
     */
    const requests = await test.pool.query(
      "SELECT 1 FROM structured.verification_requests",
    );
  ...
  it("시장 대표가격에 들어가지 않는다", async () => {
    ...
    for (
      let index = 0;
      index < PRICING_POLICY.minimumSampleCount + 1;
      index += 1
    ) {
      const { headers } = await signInAs(test, `apple-user-${index}`);
      const response = await register(headers, {
        paidAmount: 3_000_000 + index,
      });
  ...
  it("원본은 24시간 뒤에 지워진다", async () => {
    ...
    const rawDocumentId = upload.json<{ rawDocumentId: string }>()
      .rawDocumentId;
    ...
    const deletedBy = new Date(
      response.json<{ originalDeletedBy: string }>().originalDeletedBy,
    );
  ...
  it("계약서 원본은 여전히 30일이다", async () => {
    ...
    const days =
      (rows[0]!.retention_until.getTime() - Date.now()) / (24 * 60 * 60 * 1000);
...
describeWithDb("결제문자 읽기", () => {
  ...
  it("읽기만 하고 저장하지 않는다", async () => {
    ...
    expect(
      response.json<{ merchantName: { value: string } }>().merchantName.value,
    ).toBe("가온예식홀");
  ...
  it("카드번호가 응답에 실리지 않는다", async () => {
    ...
    // 종류는 온다.
    expect(
      response.json<{ maskedIdentifiers: string[] }>().maskedIdentifiers,
    ).toContain("card_number");
  ...
  it("취소 문자는 거절 이유와 함께 온다", async () => {
    ...
    const response = await parse(
      headers,
      "신한카드 승인취소 3,000,000원 05/22 가온예식홀",
    );
  ...
  it("확신이 낮은 항목을 짚어준다", async () => {
    ...
    const response = await parse(
      headers,
      "신한카드 승인 3,000,000원 05/20 14:23 가온예식홀",
    );

    expect(
      response.json<{ needsConfirmation: string[] }>().needsConfirmation,
    ).toContain("paidAt");
  });
