...
describeWithDb('관리자 로그인', () => {
  ...
  it('첫 로그인은 계정을 만들고 활성으로 표시한다', async () => {
    /*
     * **0102 전에는 여기가 403이었다.** 운영 권한은 CLI로만 켤 수 있었기 때문이다.
     *
     * 이제 `structured.admin_accounts`에 켜져 있는 슈퍼 관리자가 하나도 없으면 이
     * 환경변수 계정이 슈퍼 관리자로 보인다(부트스트랩 전용 · 2026-09-10 결정) —
     * 그래야 계정을 전부 잃은 날에도 되살릴 길이 남는다. 그 길은 슈퍼 관리자가
     * 하나 생기는 순간 닫히고, `admin-accounts.test.ts`가 그것을 본다.
     *
