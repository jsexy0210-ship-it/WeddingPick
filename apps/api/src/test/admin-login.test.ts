...
describeWithDb('관리자 로그인', () => {
  ...
  it('첫 로그인은 계정을 만들고 활성으로 표시한다', async () => {
    /*
     * **부트스트랩 자격이 곧 첫 등급이다**(#173). 예전에는 첫 로그인이 계정만 만들고
     * 403으로 돌아왔고, 운영 권한은 CLI로만 켤 수 있었다. 그러면 서버 셸에 못 들어가는
     * 사람은 만들어 놓은 콘솔에 영영 못 들어간다 — 그래서 슈퍼 관리자가 하나도 없는
     * 동안에만 이 길이 열린다. 하나 생기면 곧바로 닫힌다(`bootstrapCandidate`).
     */
