...
describeWithDb("우리웨딩", () => {
  ...
  async function mine() {
    ...
    return {
      ...session,
      weddingId: await createWedding(test, session.headers),
    };
  }
  ...
  describe("웨딩 스케줄", () => {
    ...
    it("지운 항목을 다시 깔지 않는다", async () => {
      ...
      const { headers, weddingId } = await mine();
      const first = (await tasks(headers, weddingId)).json<{
        tasks: { id: string }[];
      }>();
      ...
      const second = (await tasks(headers, weddingId)).json<{
        tasks: unknown[];
      }>();
    ...
    it("보낸 칸만 고친다", async () => {
      ...
      expect(after.tasks.find((row) => row.id === id)!.vendorLabel).toBe(
        "스튜디오 이로",
      );
    });
  ...
  describe("지출내역", () => {
    it("결제인증이 지출로 들어온다", async () => {
      ...
      expect(body.expenses[0]!.sourceLabel).toBe("결제인증");
    ...
    it("결제인증에서 온 줄은 지출내역에서 지울 수 없다", async () => {
      ...
      const proofId = registered.json<{ paymentProofId: string }>()
        .paymentProofId;
  ...
  describe("방문노트", () => {
    it("업체를 못 찾아도 적을 수 있다", async () => {
      ...
      const body = (
        ...
        notes: { vendorLabel: string; quotedAmount: number }[];
        caveat: string;
      }>();
