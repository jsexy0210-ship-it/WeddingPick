...
/**
 * 카카오 사용자 정보에서 **연령대만** 묻는다.
 *
 * 출생 연도를 받지 않는다(2026-09-10 사용자 지시). 목적은 생일을 아는 것이 아니라
 * 만 14세 이상인지 확인하는 것이고, 그 목적에 맞는 최소값이 연령대다. 출생 연도는
 * 연도만이라 생일이 지났는지 알 수 없어 만 나이가 한 살 범위로 흔들리는데,
 * 연령대의 아래끝에는 그 흔들림이 없다 — 더 적게 받고 더 정확하다.
 *
 * **못 받아도 로그인은 계속된다.** 이 호출이 실패했다고 로그인을 막으면 동의 항목
 * 설정이 바뀌는 동안 아무도 못 들어온다. 없으면 체크박스가 그대로 판정한다.
 *
