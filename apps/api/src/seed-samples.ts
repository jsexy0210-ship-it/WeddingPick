...
/**
 * 업종별 샘플 업체 20곳씩 — 화면 검수용(2026-09-08 오더). 준비 순서의 12개 업종
 * (핸드오프 v3.22 §13.6)만 넣고 «기타»는 넣지 않는다 — 사용자 화면에 업종으로
...
const RECIPES: Record<SampleCategory, Recipe> = {
  ...
  /* 헤어변형(v3.22) — 메이크업 샵과 따로 센다. 제보 금액 20~80만원. */
  hair: {
    prefixes: ['살롱드', '헤어', '오브', '라뷰', '뷰티', '아뜰리에', '온', '유', '수', '진', '연', '윤', '민', '해', '담', '결', '루엘', '비비', '리엔', '세라', '클로', '샤', '메이', '바이', '올리브', '벨', '고운', '온유', '제니', '순수'],
    suffixes: ['헤어', '헤어살롱', '헤어 아뜰리에', '헤어변형', '브라이덜 헤어'],
    keywords: 'wedding,hair,bride',
    amount: [200_000, 800_000],
  },
  snap: {
  ...
  /* 부케(v3.22) — 본식 준비. 제보 금액 10~40만원. */
  bouquet: {
    prefixes: ['청담', '꽃', '플로', '블룸', '봄', '가든', '피오니', '로즈', '릴리', '라넌', '튤립', '데이지', '온', '소소', '담다', '메종', '아뜰리에', '오드', '루나', '베르', '화이트', '아이보리', '버건디', '라벤더', '민트', '세이지', '고운', '단아', '결', '윤슬'],
    suffixes: ['부케', '플라워', '플로리스트', '꽃집', '플라워 스튜디오'],
    keywords: 'wedding,bouquet,flowers',
    amount: [100_000, 400_000],
  },
  goods: {
...
const REVIEW_TEXTS: Record<SampleCategory, { title: string; body: string }[]> = {
  ...
  hair: [
    { title: '변형이 자연스러웠어요', body: '촬영 중간에 올림머리에서 반묶음으로 바꿔 주셨는데 이음새가 티 나지 않았어요. 참고 사진의 분위기를 얼굴형에 맞게 조정해 주셨어요.' },
    { title: '하루 종일 흐트러지지 않았어요', body: '새벽에 세팅했는데 본식 끝나고 피로연까지 그대로였어요. 헤어핀과 장식 비용도 처음 안내받은 금액 그대로였어요.' },
    { title: '스타일 제안이 좋았어요', body: '드레스 사진을 보여드리니 네크라인에 맞는 스타일을 두 가지 제안해 주셨어요. 리허설 때 비교해 보고 정할 수 있어서 마음이 놓였어요.' },
    { title: '변형 속도가 빨랐어요', body: '본식과 2부 사이 시간이 짧았는데 십 분 안에 변형을 끝내 주셨어요. 신랑 헤어까지 같은 팀이 맡아서 톤이 맞았어요.' },
    { title: '요청 사항 반영이 빨라요', body: '앞머리 방향과 장식 위치를 바꿔 달라고 했더니 바로 반영해 주셨어요. 추가 비용 없이 처음 안내 그대로였어요. 응대가 친절했어요.' },
    { title: '두피가 편했어요', body: '핀을 많이 쓰는데도 하루 종일 아프지 않았어요. 스프레이 냄새도 심하지 않았고 마무리가 깔끔했어요. 준비 과정에서 물어본 것마다 답이 빨랐어요.' },
    { title: '시간 약속이 정확했어요', body: '예약 시간에 맞춰 바로 시작했고 예식장 도착 시간도 여유 있었어요. 어머님 헤어 비용도 미리 안내받아 놀랄 일이 없었어요.' },
    { title: '무난했어요', body: '시술과 응대 모두 무난했어요. 대기 시간이 조금 있었지만 진행 자체는 매끄러웠어요. 준비 과정에서 물어본 것마다 답이 빨라서 마음이 놓였어요.' },
  ],
  snap: [
  ...
  bouquet: [
    { title: '꽃이 싱싱했어요', body: '예식 당일 아침에 받았는데 저녁까지 꽃이 처지지 않았어요. 드레스 색에 맞춰 리본 색을 바꿔 주신 것도 좋았어요.' },
    { title: '디자인이 사진 그대로였어요', body: '참고로 보낸 사진과 거의 같은 구성으로 만들어 주셨어요. 부토니에와 코사지까지 같은 꽃으로 맞춰 주셔서 통일감이 있었어요.' },
    { title: '전달이 정확했어요', body: '약속한 시간에 예식장으로 직접 가져다주셨고 보관 방법도 알려 주셨어요. 준비 과정에서 물어본 것마다 답이 빨라서 마음이 놓였어요.' },
    { title: '요청 사항 반영이 좋았어요', body: '알레르기 때문에 특정 꽃을 빼 달라고 했더니 비슷한 색의 다른 꽃으로 바꿔 주셨어요. 크기도 원하는 대로 줄여 주셨어요.' },
    { title: '계절 꽃 추천이 좋았어요', body: '원하던 꽃이 철이 아니라고 미리 알려 주시고 비슷한 분위기의 계절 꽃을 추천해 주셨어요. 가격도 처음 안내와 같았어요.' },
    { title: '보존 처리까지 해줬어요', body: '예식 뒤 부케를 드라이플라워로 만들어 주는 옵션이 있어서 맡겼어요. 색이 잘 남아서 기념으로 두기 좋아요. 응대가 친절했어요.' },
    { title: '가격이 합리적이었어요', body: '비슷한 구성을 여러 곳에서 비교했는데 꽃 상태 대비 가격이 합리적이었어요. 배송비가 포함이라 계산이 편했어요.' },
    { title: '무난했어요', body: '큰 아쉬움 없이 무난했어요. 성수기라 상담 예약이 조금 어려웠지만 결과물은 안내받은 대로였어요. 준비 과정이 편했어요.' },
  ],
  goods: [
