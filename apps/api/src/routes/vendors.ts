...
async function loadVendorDetail(pool: Pool, vendorId: string, viewerId: string | null) {
   ...
   * 기다리고 있었다 — API(Render · Ohio)에서 DB(Neon · Singapore)까지 한 번
   * 다녀오는 데 199ms가 들어서, 그 순서가 그대로 1초가 됐다. 재 보면
   * 37ms + 5회 × 199ms = 1,032ms이고 실제 측정이 1,048.8ms다
   * (`docs/perf/audit-2026-09-11.md`). 묶으면 왕복 다섯 번이 한 번이 된다.
   * 질의도 조건도 바꾸지 않는다 — 순서만 바꾼다.
   *
