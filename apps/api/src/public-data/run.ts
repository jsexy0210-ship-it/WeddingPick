 ...
 * 없앴는데, 그 뒤로 한 실행이 쓸 수 있는 양에 **아무 한계가 없다.** 분류가 한 번
 ...
 *
 * 값은 실제 웨딩 업체 수보다 넉넉히 위, 분류 사고보다 확실히 아래로 둔다. 확인된
 * 소분류 다섯(README «확인된 업종 소분류 코드») 전국 합은 만 단위이고, 분류가
 * 어긋났을 때 들어올 상권 자료 전체는 백만 단위다. 출처를 넓혀 이 선이 낮아지면
 * `PUBLIC_DATA_MAX_APPLY`로 올린다 — 사람이 그 숫자를 보고 정하라는 뜻이다.
 */
