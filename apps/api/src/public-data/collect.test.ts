...
/*
 * 2026-09-14 대표 지시 — 「한복은 빼」.
 *
 * G20904는 2026-09-10 조회로 확인된 실측 코드라서, 「못 찾은 코드」를 채우다 보면
 * 다시 넣기 쉽다. 넣으면 혼수가 채워지므로 되돌린 것이 개선처럼 보이기까지 한다.
 * 그래서 목록 자체를 시험으로 고정한다 — 다시 넣으려면 이 시험을 고쳐야 하고,
 * 고치는 사람은 여기 적힌 지시를 읽게 된다.
 */
test('한복 소매업은 수집 업종코드에 넣지 않는다', () => {
  expect(WEDDING_UPJONG_CODES).not.toContain('G20904');
  // 꽃집은 남긴다(2026-09-14 대표 확정). 같은 커밋의 코드 둘 중 한복만 되돌렸다.
  expect(WEDDING_UPJONG_CODES).toContain('G21901');
});
