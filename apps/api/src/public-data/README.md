...
| M11301 | 사진촬영업 | 스튜디오 · 본식스냅 | 웨딩 · 본식 · 스냅 |
| S20701 | 미용실 | 메이크업 · 헤어변형 | 웨딩 · 브라이덜 |
| N11004 | 의류 대여업 | 드레스 | 웨딩 · 브라이덜 |
| G21901 | 꽃집 | 부케 | 웨딩 · 브라이덜 |

**한복 소매업(G20904)은 넣지 않는다 — 2026-09-14 대표 지시 「한복은 빼」.**
PR #216이 2026-09-11 「업종코드 확대」 지시로 더했던 것을 되돌린 결정이다. 한복은
웨딩 전용이 아니다. 뷔페(I20702 · I20801)도 같은 이유로 넣지 않았다.

**꽃집(G21901)은 남긴다 — 2026-09-14 대표 확정.** 부케는 우리 업종 12종에 실제로
있는 자리다. PR #216이 더한 코드 둘 중 꽃집만 살리고 한복만 되돌린 것이다.

### 들어오지 않는 업종 — 같은 0건이 아니다

| 업종 | 왜 0건인가 | 다음 수 |
| --- | --- | --- |
| 청첩장(인쇄업) · 예물(귀금속) · 허니문(여행사) | **받아올 소분류 코드를 못 찾았다.** 분류 규칙은 이미 셋을 다룬다 | 아래 조회로 찾아 넣는다 |
| 혼수(한복) | **코드는 찾았는데 대표 지시로 뺐다**(G20904) | 없다. 「아직 정보가 적어요」로 보이는 것이 맞다 |

앞의 셋은 추측으로 채우지 않는다 — 전에 대분류 `'Q'`를 박았다가 활용가이드에 없는
값이라 수집이 조용히 0건이 된 적이 있다. 아래 「업종코드를 바꿀 때는 조회로 확인한다」의
`--lookup-category`를 `--keyword 청첩` · `귀금속` · `여행`으로 돌려 찾은 뒤 여기와
`WEDDING_UPJONG_CODES`에 넣는다. `public-data.yml`은 `lookup_level=small` +
`lookup_keyword`로 같은 일을 한다.

**「확정 못 한 행을 기타로 받는다」 규칙은 아직 안 켰다.** PR #216이 같은 2026-09-11
지시로 넣었던 것인데, 켜면 이 표가 유일한 거름망이 되어 상호 조건이 사라진다.
2026-09-13 크론(run 155) 실측 — sbiz-all이 원본 145,985건 중 3,346건만 남기고
142,606건을 상호 조건으로 걸렀다. 그 규칙을 켜면 걸러지던 것이 전부 「기타」로 들어와
`MAX_APPLY` 50,000의 약 2.9배가 된다. 업종코드와 별건으로 판단이 필요하다.
...
```sh
npm run public-data:import --workspace @weddingpick/api -- --source sbiz-all \
  --upjong-div-id indsSclsCd --upjong-codes S21101 --limit 100 --out ../../.collection
```
...
리포트의 `categoryCounts`와 Actions 잡 요약은 정제·중복 제거를 통과한 업체를 **업종 12종 +
기타로 나눠** 센다. 업종별 합계는 `accepted`와 같으며, DB 반영 건수나 전국 업체 총수가 아니라
해당 실행에서 받아들인 업체 수다. 0건인 업종도 0으로 적는다 — 「코드를 안 받아와서 0」과
「받았는데 없어서 0」은 다른 이야기이고, 칸이 비어 있으면 그 둘을 구분할 수 없다. 같은 표가
워크플로 실행 페이지 요약에도 찍히므로 산출물을 내려받지 않고 업종별 숫자를 볼 수 있다.
