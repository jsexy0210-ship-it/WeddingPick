...
결혼 상담 서비스업(S21105)은 최신 디자인에서 결정사를 제외했으므로 기본 조회와
외부 업종코드 설정에서 제외한다. 이 업종 응답은 상호에 웨딩 표시가 있어도 저장하지 않는다.
...
`SBIZ_UPJONG_DIV_ID`를 생략하면 코드 형식(대분류 2자·중분류 4자·소분류 6자)으로
조회 단계를 고른다. 서로 다른 단계의 코드나 명시한 조회 단계와 맞지 않는 코드는 실패한다.
...
.github/workflows/public-data.yml이 두 크론으로 돈다.

| 크론(UTC) | 사람이 읽는 시각 | 대상 |
| --- | --- | --- |
| `17 18 * * 6` | 매주 일요일 03:17 KST | 다섯 출처 전부(`icheon-halls`·`jecheon-halls`·`sbiz-all`·`sbiz-seoul`·`sbiz-gyeonggi`) |
| `17 18 * * 2,4` | 매주 수·금 03:17 KST | `sbiz-seoul`·`sbiz-gyeonggi`만 — 나머지 출처는 그 실행에서 건너뛴다 |

**둘째 크론은 2026-09-23에 더했다**(대표 지시 「박람회·업체 정보 계속 모은다. 서울·경기
비중을 가장 높이도록」). 서울·경기가 주 3회(일 + 수 + 금), 나머지는 주 1회(일)다 — 다른
출처 수를 줄이는 대신 서울·경기 둘만 추가 빈도를 얹는 방식이라 `sbiz-all`(전국 전수, 서울·경기
포함)의 전국 커버리지는 그대로 유지된다.

**아래 첫째 문단은 이 README가 실제로는 없던 크론을 있다고 적어 온 자리였다** — 2026-09-11에
「되살렸다」고 적었지만 `public-data.yml`에는 `schedule:` 트리거 자체가 없었다(`workflow_dispatch`뿐).
2026-09-23 관리자-프론트 연결 재검증 중 발견해 실제로 되살렸다. 문서가 코드보다 먼저
「됐다」고 적힌 자리라, 이 문서 하나만 보고 「자동으로 돈다」고 판단하지 않는다 — `.yml`의
`on:` 블록을 함께 본다.
...
크론을 처음 넣을 때 적어 둔 세 조건이 다음으로 섰다.
...
리포트의 `categoryCounts`와 Actions 잡 요약은 정제·중복 제거를 통과한 업체를 업종별로 센다.
업종별 합계는 `accepted`와 같으며, 수집되지 않은 업종도 0으로 표시한다. DB 반영 건수나
전국 업체 총수가 아니라 해당 실행에서 받아들인 업체 수다.
