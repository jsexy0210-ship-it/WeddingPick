 ...
 *   G21901  꽃집                → bouquet (상호에 웨딩·브라이덜이 있어야 받는다)
 *
 * 업종 이름만으로 받는 것은 앞의 둘뿐이다. 사진관·미용실·임대업·꽃집 전체를 웨딩
 * 업체로 들이지 않는다 — 상호를 함께 본다(`classifyWeddingIndustry`).
 *
 * **한복 소매업(G20904)은 넣지 않는다 — 2026-09-14 대표 지시 「한복은 빼」.**
 * PR #216이 2026-09-11 「업종코드 확대」 지시로 더했던 것을 되돌린 결정이다.
 * 한복은 웨딩 전용이 아니다. 그래서 **혼수(dowry)는 다시 0건으로 돌아간다** —
 * 억지로 채우면 사용자가 그것을 웨딩 업체로 읽으므로 「아직 정보가 적어요」로
 * 보이는 것이 맞다. 뷔페(I20702 · I20801)도 같은 이유로 넣지 않는다.
 *
 * **꽃집(G21901)은 남긴다 — 2026-09-14 대표 확정.** 부케는 우리 업종 12종에
 * 실제로 있는 자리다. 같은 커밋의 코드 둘 중 꽃집만 살리고 한복만 되돌린 것이다.
 *
 * **들어오지 않는 업종은 둘로 나눠 적는다. 같은 0건이 아니다.**
 *   코드를 못 찾음   청첩장(인쇄업) · 예물(귀금속) · 허니문(여행사). 분류 규칙은
 *                   이미 셋을 다루지만 받아올 소분류 코드를 못 찾았다. 아래
 *                   `listIndustryCategories`를 `lookup_level=small` ·
 *                   `lookup_keyword=청첩/귀금속/여행`으로 돌려 찾은 뒤 넣는다.
 *   찾았으나 제외    혼수(한복 G20904). 2026-09-10 조회로 확인된 실측 코드인데
 *                   위 대표 지시로 뺐다. 「못 찾아서 0건」과 다른 이야기다.
 *
 * **`resolveSbizCategory`의 「확정 못 한 행을 기타로 받는다」 규칙은 아직 안 켰다.**
 * PR #216이 같은 2026-09-11 지시로 넣었던 것인데, 켜면 이 목록이 유일한 거름망이
 * 되어 상호 조건이 사라진다. 2026-09-13 크론(run 155) 실측 — sbiz-all이 원본
 * 145,985건 중 3,346건만 남기고 142,606건을 상호 조건으로 걸렀다. 그 규칙을 켜면
 * 걸러지던 것이 전부 「기타」로 들어와 run.ts의 MAX_APPLY 50,000의 약 2.9배가 된다.
 * 업종코드와 별건으로 판단이 필요하다.
 *
 ...
 */
export const WEDDING_UPJONG_CODES = ['S21101', 'S21105', 'M11301', 'S20701', 'N11004', 'G21901'] as const;
