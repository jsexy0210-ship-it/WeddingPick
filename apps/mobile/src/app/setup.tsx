 ...
 *
 * **만 14세 확인은 여기 없다.** 로그인 때 카카오 연령대로 끝난다 — 이 화면에
 * 닿았다는 것 자체가 확인을 마쳤다는 뜻이다. `completeSignup`에 보내는
 * `ageVerified: true`는 계약이 요구하는 자리를 채우는 것일 뿐이고, **서버는 이
 * 값을 보지 않는다**(로그인 때 적어둔 `age_verified`를 본다). 여기서 늘 `true`를
 * 넣는 것이 예전에 가입 관문을 무력화했다 — 서버가 그 값을 믿었기 때문이고,
 * 지금은 믿지 않는다(2026-09-10). 2026-09-04 정책(비회원 진입 삭제)으로 이
