...
export default function SearchScreen() {
              ...
              {top3.items.map((item, idx) => (
                <View key={item.vendorId}>
                  <Pressable
                    ...
                    onPress={() =>
                      router.push({
                        pathname: `/search/${item.vendorId}`,
                        params: { reasons: item.reasons.join(',') },
                      })
                    }>
  ...
  function renderVendorCard(item: VendorSummary) {
    ...
    const isCollecting = item.paidPrice.stage === 'collecting';
    const isLimited = item.paidPrice.stage === 'limited';
