 ...
 *
 * **진입은 MY > 함께 준비하기 > 준비 현황 한 곳이다**(2026-09-15 MASTER). 원래는 홈
 * 4칸의 «전체 보기»가 오는 자리였는데, 홈 전면 개편(§1)이 준비현황 2×2를 Pick 추천에
 * 흡수하면서 그 단추가 없어졌다 — 화면만 남고 들어갈 길이 사라진 상태였다.
 *
 * **왜 MY인가.** 홈으로 되돌리는 것은 대표 사양이 홈에서 뺀 것을 다시 넣는 일이고,
 * 웨딩노트는 피그마가 탭 셋(캘린더 · 상담기록 · 예산현황)으로 못 박은 자리라 넷째 칸을
 * 만들 수 없다. MY의 «함께 준비하기»는 이미 «내가 어디까지 왔나»를 모아 두는 자리다.
 *
 * **`/recommendations`와 다른 화면이다.** 여기는 열두 업종을 «끝냈다 · 고르는 중 · 아직»
 * 으로 묶어 보여주는 **상태**이고, 추천 전체는 «다음에 무엇을 고를까»의 **업체 목록**이다.
 * 하나로 합치면 이미 정한 업종이 목록에서 사라져 «내가 무엇을 끝냈는지»를 볼 곳이 없다.
 *
 * 웨딩일정 메뉴의 준비현황(WP-OUR-002)은 폐기했다 — 같은 내용을 두 탭에서 보여주면
 * 사용자가 어디서 봐야 할지 헷갈린다. 진입이 하나라는 규칙은 그대로다.
 *
