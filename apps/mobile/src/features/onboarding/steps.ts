...
/**
 * 온보딩 4장. 디자인 핸드오프 1번이 문구와 예시 카드까지 정했다.
 *
 * 화면이 아니라 자료로 둔 이유는, 이 네 장이 서비스가 무엇을 하는지 말하는
 * 유일한 자리이기 때문이다 — 문구가 화면 코드 사이에 흩어지면 한 장만 고쳐지고
 * 나머지는 옛말을 계속한다.
 *
 * **다섯 장이었다.** 마지막 「홈은 원하는 순서로 바꿀 수 있어요」 장을 2026-09-15
 * 대표 지시로 통째로 뺐다 — 「통으로 뺀다」. 장 수를 세는 곳은 전부 이 배열의
 * 길이를 읽으므로(점 · `isLast` · `ctaLabel`) 여기만 고치면 된다.
 *
