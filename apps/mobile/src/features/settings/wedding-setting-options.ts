 ...
 * 날짜 외에는 1열 휠」.
 *
 * **정본에는 이 화면의 편집 시트 그림이 없다**(my.jsx frame-003은 다섯 행만 그린다). 그래서
 * 보기는 새로 짓지 않고 **온보딩이 받는 답 그대로**를 1열로 늘어놓는다 — 보기 묶음 자체가
 * `DESIGN_UNRESOLVED`다.
 *
 *   예산      온보딩 4/5가 서버에 보내는 여섯 구간(`WEDDING_BUDGET_BRACKETS`) 그대로
...
export function budgetOptions(): readonly WheelOption<WeddingBudgetBracket>[] {
  return WEDDING_BUDGET_BRACKETS.map((bracket) => ({ value: bracket, label: BUDGET_BRACKET_LABEL[bracket] }));
}
