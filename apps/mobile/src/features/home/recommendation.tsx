  ...
  return (
    <ThemedView style={styles.section}>
      ...
      {main === undefined ? (
        ...
        <MainCard
          vendor={main}
          picked={isPicked(main.id)}
          onPress={() => onPressVendor(main.id)}
          onPressPick={() => onPressPick(main)}
        />
      )}
...
function MainCard({ vendor, picked, onPress, onPressPick }: {
  ...
  return (
    <Pressable
      ...
      <View style={styles.mainImage}>
        ...
        <PickButton vendor={vendor} picked={picked} onPress={onPressPick} />
      </View>
...
function SubCard({ vendor, picked, onPress, onPressPick }: {
  vendor: VendorSummary;
  picked: boolean;
  onPress: () => void;
  onPressPick: () => void;
...
function PickButton({ vendor, picked, onPress }: {
  vendor: VendorSummary;
  picked: boolean;
  onPress: () => void;
}) {
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.pick,
        { backgroundColor: picked ? theme.text : theme.background },
        pressed && styles.pressed,
      ]}>
      <SeedIcon
...
const styles = StyleSheet.create({
  ...
  mainImage: { width: '100%', height: 180, borderRadius: Radius.medium, overflow: 'hidden', position: 'relative' },
  ...
  subImage: { width: '100%', aspectRatio: 16 / 11, borderRadius: Radius.small, overflow: 'hidden', position: 'relative' },
  pick: {
    position: 'absolute',
    top: Layout.cardGap,
