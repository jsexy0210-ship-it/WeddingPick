 ...
 * ```
 *
 * **라운지가 내려가고 검색이 올라왔다.** 근거는 옛 `docs/design/figma-export/README.md`(2026-09-22
 * 파기, 이 저장소에 없음) — 「라운지는 탭이 아닙니다. 초기에는 후기와 박람회가 몇 건뿐이라 탭
 * 한 칸이 빈 화면을 띄웁니다」. 검색은 2026-09-14에 「초기 이미지 데이터가 없어서」 임시로
 * 내렸던 것이고 그때부터 «차후에 되돌린다»고 적어 둔 자리다 — 지금이 그때다. **결정 자체는
 * 지금도 유효하다** — 현 정본 `docs/design/README.md`도 Root 탭을 홈 · 검색 · Pick · 웨딩노트 ·
 * MY 다섯으로 두고, CLAUDE.md도 같다.
 *
